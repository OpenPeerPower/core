...
async def test_check_loop_async_integration(caplog):
    ...
    with patch(
        ...
        hasync.check_loop()
    assert (
        "Detected I/O inside the event loop. This is causing stability issues. Please report issue for hue doing I/O at openpeerpower/components/hue/light.py, line 23: self.light.is_on"
        in caplog.text
    )
