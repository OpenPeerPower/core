...
def update_events(opp):
    ...
    opp.bus.async_listen(er.EVENT_ENTITY_REGISTRY_UPDATED, async_capture)
...
async def test_loading_saving_data(opp, registry):
    ...
    registry2 = er.EntityRegistry(opp)
...
async def test_disable_config_entry_disables_entities(opp, registry):
    ...
    await opp.config_entries.async_set_disabled_by(
...
async def test_entity_max_length_exceeded(opp, registry):
