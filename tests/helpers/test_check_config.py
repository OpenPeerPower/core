...
from openpeerpowerr.helpers.check_config import (
    CheckConfigError,
    async_check_op.config_file,
)
...
async def test_bad_core_config.opp):
    ...
    with patch("os.path.isfile", return_value=True), patch_yaml_files(files):
        res = await async_check_op.config_file.opp)
        log_op.config(res)

        assert isinstance(res.errors[0].message, str)
        assert res.errors[0].domain == "openpeerpowerr"
...
async def test_config_platform_valid.opp):
    ...
    with patch("os.path.isfile", return_value=True), patch_yaml_files(files):
        ...
        assert res.keys() == {"openpeerpowerr", "light"}
...
async def test_component_platform_not_found.opp):
    ...
    with patch("os.path.isfile", return_value=True), patch_yaml_files(files):
        ...
        assert res.keys() == {"openpeerpowerr"}
...
async def test_package_invalid.opp):
    ...
    with patch("os.path.isfile", return_value=True), patch_yaml_files(files):
        ...
        assert res.errors[0].domain == "openpeerpowerr.packages.p1.group"
