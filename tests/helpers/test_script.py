...
from openpeerpower.core import SERVICE_CALL_LIMIT, Context, CoreState, callback
from openpeerpower.exceptions import ConditionError, ServiceNotFound
from openpeerpower.helpers import config_validation as cv, script, trace
...
async def test_repeat_count(opp, caplog, count):
...
async def test_breakpoints_1(opp):
    ...
    script.breakpoint_set(opp, "script_1", script.RUN_ID_ANY, "1")
    script.breakpoint_set(opp, "script_1", script.RUN_ID_ANY, "5")
    ...
    async_dispatcher_connect(opp, script.SCRIPT_BREAKPOINT_HIT, breakpoint_hit)
    ...
    script.debug_step(opp, "script_1", "1")
    await breakpoint_hit_event.wait()
    assert script_obj.is_running
    assert script_obj.runs == 1
    assert len(events) == 2
    ...
    breakpoint_hit_event.clear()
    script.debug_continue(opp, "script_1", "1")
    await breakpoint_hit_event.wait()
    assert script_obj.is_running
    assert script_obj.runs == 1
    assert len(events) == 5
...
async def test_breakpoints_2(opp):
    ...
    # Abort script
    script.debug_stop(opp, "script_1", "1")
    await opp.async_block_till_done()
    assert not script_obj.is_running
    assert script_obj.runs == 0
    assert len(events) == 1
