...
async def test_device_info.opp):
...
async def test_device_not_accessible.opp):
...
async def test_new_event_sends_signal.opp):
    ...
    axis_device = axis.device.AxisNetworkDevice.opp, entry)
...
async def test_get_device_fails.opp):
