...
import openpeerpower.components.tcp.common as tcp
...
def mock_socket_fixture(mock_select):
    """Mock socket."""
    with patch("openpeerpower.components.tcp.common.socket.socket") as mock_socket:
...
def mock_ssl_context_fixture():
    """Mock select."""
    with patch(
        "openpeerpower.components.tcp.common.ssl.create_default_context",
    ) as mock_ssl_context:
...
async def test_ssl_state(opp, mock_socket, mock_select, mock_ssl_context):
...
async def test_ssl_state_verify_off(opp, mock_socket, mock_select, mock_ssl_context):
