...
async def test_state_without_turnon(opp, remote):
    ...
    with patch(
        "openpeerpower.components.samsungtv.bridge.Remote",
        side_effect=OSError,
    ), patch("openpeerpower.util.dt.utcnow", return_value=next_update):
        async_fire_time_changed(opp, next_update)
        await opp.async_block_till_done()
...
async def test_turn_on_wol(opp, remotews):
    """Test turn on."""
    entry = MockConfigEntry(
        domain=SAMSUNGTV_DOMAIN,
    ...
    await opp.async_block_till_done()
    with patch(
        "openpeerpower.components.samsungtv.media_player.send_magic_packet"
