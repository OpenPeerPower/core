...
from openpeerpower.const import ATTR_ASSUMED_STATE, STATE_UNKNOWN
...
async def test_nested_sensor_state_via_mqtt(opp, mqtt_mock, setup_tasmota):
    """Test state update via MQTT."""
    config = copy.deepcopy(DEFAULT_CONFIG)
...
async def test_indexed_sensor_state_via_mqtt(opp, mqtt_mock, setup_tasmota):
    """Test state update via MQTT."""
    config = copy.deepcopy(DEFAULT_CONFIG)
...
async def test_indexed_sensor_state_via_mqtt2(opp, mqtt_mock, setup_tasmota):
    ...
    state = opp.states.get("sensor.tasmota_energy_total")
    assert state.state == "unavailable"
    assert not state.attributes.get(ATTR_ASSUMED_STATE)
...
async def test_bad_indexed_sensor_state_via_mqtt(opp, mqtt_mock, setup_tasmota):
    ...
    state = opp.states.get("sensor.tasmota_energy_apparentpower_0")
    assert state.state == "unavailable"
    assert not state.attributes.get(ATTR_ASSUMED_STATE)
    state = opp.states.get("sensor.tasmota_energy_apparentpower_1")
    assert state.state == "unavailable"
    assert not state.attributes.get(ATTR_ASSUMED_STATE)
    state = opp.states.get("sensor.tasmota_energy_apparentpower_2")
    assert state.state == "unavailable"
    assert not state.attributes.get(ATTR_ASSUMED_STATE)
    ...
    # Test periodic state update
    async_fire_mqtt_message(
        opp, "tasmota_49A3BC/tele/SENSOR", '{"ENERGY":{"ApparentPower":[1.2,3.4,5.6]}}'
    )
    state = opp.states.get("sensor.tasmota_energy_apparentpower_0")
    assert state.state == "1.2"
    state = opp.states.get("sensor.tasmota_energy_apparentpower_1")
    ...
    async_fire_mqtt_message(
        opp, "tasmota_49A3BC/tele/SENSOR", '{"ENERGY":{"ApparentPower":[7.8,9.0]}}'
    )
    state = opp.states.get("sensor.tasmota_energy_apparentpower_0")
    assert state.state == "7.8"
    state = opp.states.get("sensor.tasmota_energy_apparentpower_1")
    ...
    async_fire_mqtt_message(
        opp, "tasmota_49A3BC/tele/SENSOR", '{"ENERGY":{"ApparentPower":2.3}}'
    )
    state = opp.states.get("sensor.tasmota_energy_apparentpower_0")
...
async def test_status_sensor_state_via_mqtt(opp, mqtt_mock, setup_tasmota):
    """Test state update via MQTT."""
...
@pytest.mark.parametrize("status_sensor_disabled", [False])
async def test_single_shot_status_sensor_state_via_mqtt(opp, mqtt_mock, setup_tasmota):
    """Test state update via MQTT."""
    entity_reg = er.async_get(opp)
...
async def test_attributes(opp, mqtt_mock, setup_tasmota):
    ...
    state = opp.states.get("sensor.tasmota_beer_CarbonDioxide")
...
async def test_nested_sensor_attributes(opp, mqtt_mock, setup_tasmota):
    """Test correct attributes for sensors."""
    config = copy.deepcopy(DEFAULT_CONFIG)
    sensor_config = copy.deepcopy(NESTED_SENSOR_CONFIG)
    mac = config["mac"]
    ...
    state = opp.states.get("sensor.tasmota_tx23_dir_avg")
    assert state.attributes.get("device_class") is None
...
async def test_indexed_sensor_attributes(opp, mqtt_mock, setup_tasmota):
    """Test correct attributes for sensors."""
    config = copy.deepcopy(DEFAULT_CONFIG)
    sensor_config = {
        "sn": {
    ...
    state = opp.states.get("sensor.tasmota_dummy2_carbondioxide_1")
    assert state.attributes.get("device_class") == "carbon_dioxide"
...
@pytest.mark.parametrize("status_sensor_disabled", [False])
async def test_enable_status_sensor(opp, mqtt_mock, setup_tasmota):
...
async def test_discovery_removal_sensor(opp, mqtt_mock, caplog, setup_tasmota):
