...
from openpeerpower.components.tplink.light import SLEEP_TIME
...
def unknown_light_mock_data_fixture() -> None:
    ...
    set_light_state_patch = patch(
        "openpeerpower.components.tplink.common.SmartBulb.set_light_state",
        side_effect=set_light_state,
    )
    get_light_state_patch = patch(
        "openpeerpower.components.tplink.common.SmartBulb.get_light_state",
        return_value=light_state,
    )
    current_consumption_patch = patch(
        "openpeerpower.components.tplink.common.SmartDevice.current_consumption",
        return_value=3.23,
    )
    ...
    get_emeter_daily_patch = patch(
        "openpeerpower.components.tplink.common.SmartDevice.get_emeter_daily",
        return_value={
            1: 1.01,
            2: 1.02,
            3: 1.03,
    ...
    get_emeter_monthly_patch = patch(
        "openpeerpower.components.tplink.common.SmartDevice.get_emeter_monthly",
        return_value={
            1: 2.01,
            2: 2.02,
            3: 2.03,
...
def dimmer_switch_mock_data_fixture() -> None:
    ...
    state_patch = patch(
        "openpeerpower.components.tplink.common.SmartPlug.state",
        new_callable=PropertyMock,
        side_effect=state,
    )
    brightness_patch = patch(
        "openpeerpower.components.tplink.common.SmartPlug.brightness",
        new_callable=PropertyMock,
...
async def test_unknown_light(
    opp: OpenPeerPower, unknown_light_mock_data: LightMockData
) -> None:
    """Test function."""
    await async_setup_component(opp, HA_DOMAIN, {})
    await opp.async_block_till_done()
...
async def test_update_failure(
    ...
    with patch("openpeerpower.components.tplink.light.MAX_ATTEMPTS", 0):
        caplog.clear()
        caplog.set_level(logging.WARNING)
        await opp.helpers.entity_component.async_update_entity("light.light1")
    ...
    with patch("openpeerpower.components.tplink.light", MAX_ATTEMPTS=2, SLEEP_TIME=0):
        caplog.clear()
