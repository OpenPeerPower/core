...
def patch_mydevolo(request):
    ...
    with patch(
        "openpeerpower.components.devolo_home_control.Mydevolo.credentials_valid",
        return_value=not bool(request.node.get_closest_marker("credentials_invalid")),
    ), patch(
        "openpeerpower.components.devolo_home_control.Mydevolo.maintenance",
        return_value=bool(request.node.get_closest_marker("maintenance")),
    ), patch(
        "openpeerpower.components.devolo_home_control.Mydevolo.get_gateway_ids",
