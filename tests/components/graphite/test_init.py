...
import openpeerpower.components.graphite as graphite
...
class TestGraphite(unittest.TestCase):
    ...
    def setup_method(self, method):
        ...
        self.gf = graphite.GraphiteFeeder(self.opp, "foo", 123, "tcp", "ha")
    ...
    def test_setup(self, mock_socket):
        """Test setup."""
        assert setup_component(self.opp, graphite.DOMAIN, {"graphite": {}})
        assert mock_socket.call_count == 1
        assert mock_socket.call_args == mock.call(socket.AF_INET, socket.SOCK_STREAM)

    @patch("socket.socket")
    @patch("openpeerpower.components.graphite.GraphiteFeeder")
    def test_full_config(self, mock_gf, mock_socket):
        """Test setup with full configuration."""
        config = {"graphite": {"host": "foo", "port": 123, "prefix": "me"}}

        assert setup_component(self.opp, graphite.DOMAIN, config)
        assert mock_gf.call_count == 1
        assert mock_gf.call_args == mock.call(self.opp, "foo", 123, "tcp", "me")
        assert mock_socket.call_count == 1
        assert mock_socket.call_args == mock.call(socket.AF_INET, socket.SOCK_STREAM)
    ...
    def test_full_udp_config(self, mock_gf, mock_socket):
        ...
        assert setup_component(self.opp, graphite.DOMAIN, config)
        assert mock_gf.call_count == 1
        assert mock_gf.call_args == mock.call(self.opp, "foo", 123, "udp", "me")
    ...
    def test_subscribe(self):
        ...
        fake_opp = mock.MagicMock()
        gf = graphite.GraphiteFeeder(fake_opp, "foo", 123, "tcp", "ha")
        fake_opp.bus.listen_once.has_calls(
            [
                mock.call(EVENT_OPENPEERPOWER_START, gf.start_listen),
                mock.call(EVENT_OPENPEERPOWER_STOP, gf.shutdown),
            ]
        )
        assert fake_opp.bus.listen.call_count == 1
        assert fake_opp.bus.listen.call_args == mock.call(
