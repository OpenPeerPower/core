...
async def test_simple_climate_device(opp, aioclient_mock, mock_deconz_websocket):
...
async def test_climate_device_with_preset(opp, aioclient_mock, mock_deconz_websocket):
    """Test successful creation of sensor entities."""
    data = {
        "sensors": {
            "0": {
...
async def test_clip_climate_device(opp, aioclient_mock):
    ...
    assert len(opp.states.async_all()) == 2
    assert not opp.states.get("climate.clip_thermostat")
...
async def test_verify_state_update(opp, aioclient_mock, mock_deconz_websocket):
...
async def test_add_new_climate_device(opp, aioclient_mock, mock_deconz_websocket):
