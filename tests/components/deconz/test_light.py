...
async def test_lights_and_groups(opp, aioclient_mock, mock_deconz_websocket):
    ...
    assert opp.states.get("light.light_group").attributes["all_on"] is False
    ...
    assert opp.states.get("light.rgb_light").state == STATE_OFF
...
async def test_disable_light_groups(opp, aioclient_mock):
    ...
    assert not opp.states.get("light.light_group")
    assert not opp.states.get("light.empty_group")
...
async def test_ikea_default_transition_time(opp, aioclient_mock):
...
async def test_verify_group_supported_features(opp, aioclient_mock):
    ...
    assert opp.states.get("light.group").state == STATE_ON
    assert opp.states.get("light.group").attributes[ATTR_SUPPORTED_FEATURES] == 63
