...
from openpeerpower.components.deconz.sensor import ATTR_DAYLIGHT
...
async def test_sensors(opp, aioclient_mock, mock_deconz_websocket):
    ...
    light_level_temp = opp.states.get("sensor.light_level_sensor_temperature")
    assert light_level_temp.state == "0.1"
    assert light_level_temp.attributes[ATTR_DEVICE_CLASS] == DEVICE_CLASS_TEMPERATURE

    assert not opp.states.get("sensor.presence_sensor")
    assert not opp.states.get("sensor.switch_1")
    assert not opp.states.get("sensor.switch_1_battery_level")
    assert not opp.states.get("sensor.switch_2")
    ...
    assert not opp.states.get("sensor.daylight_sensor")
    ...
    assert not opp.states.get("sensor.clip_light_level_sensor")
    ...
    assert opp.states.get("sensor.light_level_sensor_temperature").state == "1.0"
...
async def test_add_new_sensor(opp, aioclient_mock, mock_deconz_websocket):
...
async def test_add_battery_later(opp, aioclient_mock, mock_deconz_websocket):
    ...
    assert opp.states.get("sensor.switch_1_battery_level").state == "50"


async def test_special_danfoss_battery_creation(opp, aioclient_mock):
    ...
    assert len(opp.states.async_entity_ids(SENSOR_DOMAIN)) == 5
...
async def test_air_quality_sensor(opp, aioclient_mock):
    ...
    assert len(opp.states.async_all()) == 1
    assert opp.states.get("sensor.air_quality").state == "poor"


async def test_daylight_sensor(opp, aioclient_mock):
    ...
    assert len(opp.states.async_all()) == 1
    assert opp.states.get("sensor.daylight_sensor")
    assert opp.states.get("sensor.daylight_sensor").attributes[ATTR_DAYLIGHT]
...
async def test_time_sensor(opp, aioclient_mock):
    ...
    assert len(opp.states.async_all()) == 2
    assert opp.states.get("sensor.time").state == "2020-11-19T08:07:08Z"
    assert opp.states.get("sensor.time_battery_level").state == "40"
...
async def test_unsupported_sensor(opp, aioclient_mock):
    ...
    assert len(opp.states.async_all()) == 1
    assert opp.states.get("sensor.name").state == STATE_UNKNOWN
