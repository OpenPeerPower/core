...
async def test_binary_sensors(opp, aioclient_mock, mock_deconz_websocket):
    ...
    presence_temp = opp.states.get("sensor.presence_sensor_temperature")
    ...
    vibration_temp = opp.states.get("sensor.vibration_sensor_temperature")
...
async def test_tampering_sensor(opp, aioclient_mock, mock_deconz_websocket):
    ...
    presence_tamper = opp.states.get("binary_sensor.presence_sensor_tampered")
    ...
    assert opp.states.get("binary_sensor.presence_sensor_tampered").state == STATE_ON

    await opp.config_entries.async_unload(config_entry.entry_id)

    assert (
        opp.states.get("binary_sensor.presence_sensor_tampered").state
        == STATE_UNAVAILABLE
    )
...
async def test_allow_clip_sensor(opp, aioclient_mock):
    ...
    with patch.dict(DECONZ_WEB_REQUEST, data):
        config_entry = await setup_deconz_integration(
            opp, aioclient_mock, options={CONF_ALLOW_CLIP_SENSOR: True}
        )
    ...
    assert not opp.states.get("binary_sensor.clip_presence_sensor")
...
async def test_add_new_binary_sensor(opp, aioclient_mock, mock_deconz_websocket):
