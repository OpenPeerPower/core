...
def test_get_states(opp_recorder):
    """Test getting states at a specific point in time."""
    opp = opp_recorder()
    states = []

    now = dt_util.utcnow()
    with patch("openpeerpower.components.recorder.dt_util.utcnow", return_value=now):
        for i in range(5):
            state = ha.State(
            ...
            mock_state_change_event(opp, state)
    ...
    with patch("openpeerpower.components.recorder.dt_util.utcnow", return_value=future):
        for i in range(5):
            state = ha.State(
                f"test.point_in_time_{i % 5}",
                f"State {i}",
    ...
    for state1, state2 in zip(
        states,
        sorted(history.get_states(opp, future), key=lambda state: state.entity_id),
    ):
...
def test_state_changes_during_period(opp_recorder):
    ...
    def set_state(state):
        """Set the state."""
        opp.states.set(entity_id, state)
        wait_recording_done(opp)
    ...
    with patch("openpeerpower.components.recorder.dt_util.utcnow", return_value=point):
        states = [
            set_state("idle"),
            set_state("Netflix"),
            set_state("Plex"),
    ...
    with patch("openpeerpower.components.recorder.dt_util.utcnow", return_value=end):
        set_state("Netflix")
        set_state("Plex")

    hist = history.state_changes_during_period(opp, start, end, entity_id)

    assert states == hist[entity_id]


def test_get_last_state_changes(opp_recorder):
    ...
    with patch("openpeerpower.components.recorder.dt_util.utcnow", return_value=point2):
        states.append(set_state("3"))

    hist = history.get_last_state_changes(opp, 2, entity_id)

    assert states == hist[entity_id]


def test_ensure_state_can_be_copied(opp_recorder):
...
def test_get_significant_states(opp_recorder):
...
def test_get_significant_states_minimal_response(opp_recorder):
    """Test that only significant states are returned.
    ...
    opp = opp_recorder()
    zero, four, states = record_states(opp)
    hist = history.get_significant_states(opp, zero, four, minimal_response=True)
...
def test_get_significant_states_with_initial(opp_recorder):
    """Test that only significant states are returned.
...
def test_get_significant_states_without_initial(opp_recorder):
    """Test that only significant states are returned.
...
def test_get_significant_states_multiple_entity_ids(opp_recorder):
    """Test that only significant states are returned for one entity."""
    opp = opp_recorder()
    zero, four, states = record_states(opp)
    del states["media_player.test2"]
