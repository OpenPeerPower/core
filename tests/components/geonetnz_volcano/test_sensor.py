...
from openpeerpower.components import geonetnz_volcano
from openpeerpower.components.geo_location import ATTR_DISTANCE
from openpeerpower.components.geonetnz_volcano import DEFAULT_SCAN_INTERVAL
from openpeerpower.components.geonetnz_volcano.const import (
    ATTR_ACTIVITY,
    ATTR_EXTERNAL_ID,
    ATTR_HAZARDS,
)
...
async def test_setup_opp, legacy_patchable_time):
    ...
    with patch("openpeerpower.util.dt.utcnow", return_value=utcnow), patch(
        ...
        state = opp.states.get("sensor.volcano_title_1")
        ...
        state = opp.states.get("sensor.volcano_title_2")
        assert state is not None
        ...
        state = opp.states.get("sensor.volcano_title_3")
        assert state is not None
