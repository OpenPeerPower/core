...
from openpeerpower.components.denonavr.media_player import (
    ATTR_COMMAND,
    ATTR_DYNAMIC_EQ,
    SERVICE_GET_COMMAND,
...
def client_fixture():
    ...
    with patch(
        "openpeerpower.components.denonavr.receiver.DenonAVR",
        autospec=True,
...
async def test_dynamic_eq(opp, client):
    ...
    # Verify on call
    await opp.services.async_call(DOMAIN, SERVICE_SET_DYNAMIC_EQ, data)
    await opp.async_block_till_done()
...
async def test_update_audyssey(opp, client):
    """Test that dynamic eq method works."""
    await setup_denonavr(opp)
