...
from openpeerpower.components.hyperion import get_hyperion_device_id
...
async def test_camera_setup(opp: OpenPeerPower) -> None:
    ...
    entity_state = opp.states.get(TEST_CAMERA_ENTITY_ID)
    assert entity_state
    assert entity_state.state == "idle"


async def test_camera_image(opp: OpenPeerPower) -> None:
    ...
    get_image_coro = async_get_image(opp, TEST_CAMERA_ENTITY_ID)
...
async def test_camera_invalid_image(opp: OpenPeerPower) -> None:
    ...
    get_image_coro = async_get_image(opp, TEST_CAMERA_ENTITY_ID, timeout=0)
    image_stream_update_coro = async_call_registered_callback(
...
async def test_camera_image_failed_start_stream_call(opp: OpenPeerPower) -> None:
    ...
    with pytest.raises(OpenPeerPowerError):
        await async_get_image(opp, TEST_CAMERA_ENTITY_ID, timeout=0)

    assert client.async_send_image_stream_start.called
    assert not client.async_send_image_stream_stop.called


async def test_camera_stream(opp: OpenPeerPower) -> None:
    ...
    with patch(
        "openpeerpower.components.hyperion.camera.async_get_still_stream",
    ) as fake:
        fake.side_effect = fake_get_still_stream

        get_stream_coro = async_get_mjpeg_stream(opp, request, TEST_CAMERA_ENTITY_ID)
        image_stream_update_coro = async_call_registered_callback(
            client, "ledcolors-imagestream-update", TEST_IMAGE_UPDATE
        )
...
async def test_camera_stream_failed_start_stream_call(opp: OpenPeerPower) -> None:
    ...
    request = Mock()
    assert not await async_get_mjpeg_stream(opp, request, TEST_CAMERA_ENTITY_ID)
