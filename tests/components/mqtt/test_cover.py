...
from openpeerpower.components.mqtt.const import CONF_STATE_TOPIC
from openpeerpower.components.mqtt.cover import (
...
async def test_state_via_template_and_entity_id(opp, mqtt_mock):
    """Test the controlling state via topic."""
    assert await async_setup_component(
        opp,
        cover.DOMAIN,
    ...
    async_fire_mqtt_message(opp, "state-topic", "invalid")
...
async def test_state_via_template_with_json_value(opp, mqtt_mock, caplog):
    ...
    async_fire_mqtt_message(opp, "state-topic", '{ "Var1": "open", "Var2": "other" }')
    ...
    async_fire_mqtt_message(opp, "state-topic", '{ "Var2": "other" }')
    assert (
        "Template variable warning: 'dict object' has no attribute 'Var1' when rendering"
    ) in caplog.text


async def test_position_via_template_and_entity_id(opp, mqtt_mock):
    """Test the controlling state via topic."""
    assert await async_setup_component(
        opp,
        cover.DOMAIN,
    ...
    async_fire_mqtt_message(opp, "get-position-topic", "10")
...
async def test_set_position_templated(
    opp, mqtt_mock, pos_template, pos_call, pos_message
):
...
async def test_set_position_templated_and_attributes(opp, mqtt_mock):
...
async def test_set_tilt_templated(opp, mqtt_mock):
...
async def test_set_tilt_templated_and_attributes(opp, mqtt_mock):
...
async def test_no_payload_close(opp, mqtt_mock):
    ...
    assert opp.states.get("cover.test").attributes["supported_features"] == 9


async def test_no_payload_open(opp, mqtt_mock):
    ...
    assert opp.states.get("cover.test").attributes["supported_features"] == 10
...
async def test_tilt_via_topic_template_json_value(opp, mqtt_mock, caplog):
    ...
    async_fire_mqtt_message(opp, "tilt-status-topic", '{"Var1": 9, "Var2": 30}')
    ...
    async_fire_mqtt_message(opp, "tilt-status-topic", '{"Var1": 50, "Var2": 10}')
    ...
    async_fire_mqtt_message(opp, "tilt-status-topic", '{"Var2": 10}')
...
async def test_tilt_status_out_of_range_warning(opp, caplog, mqtt_mock):
    ...
    async_fire_mqtt_message(opp, "tilt-status-topic", "60")
...
async def test_tilt_status_not_numeric_warning(opp, caplog, mqtt_mock):
    ...
    async_fire_mqtt_message(opp, "tilt-status-topic", "abc")

    assert ("Payload 'abc' is not numeric") in caplog.text


async def test_tilt_via_topic_altered_range_inverted(opp, mqtt_mock):
...
async def test_position_via_position_topic_template_json_value(opp, mqtt_mock, caplog):
    ...
    async_fire_mqtt_message(opp, "get-position-topic", '{"Var1": 9, "Var2": 60}')
    ...
    async_fire_mqtt_message(opp, "get-position-topic", '{"Var1": 50, "Var2": 10}')
    ...
    async_fire_mqtt_message(opp, "get-position-topic", '{"Var2": 60}')
...
async def test_position_template_with_entity_id(opp, mqtt_mock):
    """Test position by updating status via position template."""
    assert await async_setup_component(
        opp,
        cover.DOMAIN,
...
async def test_position_via_position_topic_template_return_json(opp, mqtt_mock):
    ...
    async_fire_mqtt_message(opp, "get-position-topic", "55")
...
async def test_position_and_tilt_via_position_topic_template_return_json(
    ...
    current_tilt_position = opp.states.get("cover.test").attributes[
        ATTR_CURRENT_TILT_POSITION
    ]
...
async def test_position_via_position_topic_template_all_variables(opp, mqtt_mock):
    """Test position by updating status via position template."""
    assert await async_setup_component(
        opp,
        cover.DOMAIN,
...
async def test_value_template_without_state_topic_error(opp, caplog, mqtt_mock):
...
async def test_position_template_without_position_topic_error(opp, caplog, mqtt_mock):
