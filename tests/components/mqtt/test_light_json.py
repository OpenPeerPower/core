...
async def test_fail_setup_if_color_mode_deprecated(opp, mqtt_mock, deprecated):
...
async def test_controlling_state_via_topic(opp, mqtt_mock):
    ...
    async_fire_mqtt_message(opp, "test_light_rgb", '{"state":"ON", "color":null}')
    ...
    async_fire_mqtt_message(opp, "test_light_rgb", '{"state":"ON", "color_temp":null}')
...
async def test_controlling_state_via_topic2(opp, mqtt_mock, caplog):
    ...
    async_fire_mqtt_message(
        opp, "test_light_rgb", '{"state":"ON", "effect":"other_effect"}'
    )
    state = opp.states.get("light.test")
    ...
    # Invalid color mode
    async_fire_mqtt_message(
        opp, "test_light_rgb", '{"state":"ON", "color_mode":"col_temp"}'
    )
...
async def test_sending_mqtt_commands_and_optimistic(opp, mqtt_mock):
    ...
    await common.async_turn_on(opp, "light.test", color_temp=90)
...
async def test_sending_mqtt_commands_and_optimistic2(opp, mqtt_mock):
    ...
    # Set hs color
    await common.async_turn_on(opp, "light.test", brightness=75, hs_color=[359, 78])
    state = opp.states.get("light.test")
    assert state.state == STATE_ON
    ...
    # Set rgbw color
    await common.async_turn_on(
        opp, "light.test", rgbw_color=[255, 128, 0, 123], white_value=80
    )
    state = opp.states.get("light.test")
    assert state.state == STATE_ON
    assert state.attributes["brightness"] == 75
    ...
    # Set rgbww color
    await common.async_turn_on(opp, "light.test", rgbww_color=[255, 128, 0, 45, 32])
    state = opp.states.get("light.test")
    assert state.state == STATE_ON
    assert state.attributes["brightness"] == 75
    ...
    # Set xy color
    await common.async_turn_on(
        opp, "light.test", brightness=50, xy_color=[0.123, 0.223]
    )
    state = opp.states.get("light.test")
    assert state.state == STATE_ON
    assert state.attributes["brightness"] == 50
...
async def test_sending_rgb_color_no_brightness2(opp, mqtt_mock):
    """Test light.turn_on with hs color sends rgb color parameters."""
    ...
    await common.async_turn_on(
        opp, "light.test", rgbw_color=[128, 64, 32, 16], brightness=128
    )
    await common.async_turn_on(
        opp, "light.test", rgbww_color=[128, 64, 32, 16, 8], brightness=64
    )
...
async def test_flash_short_and_long(opp, mqtt_mock):
    ...
    await common.async_turn_off(opp, "light.test", flash="short")
    ...
    await common.async_turn_off(opp, "light.test", flash="long")
...
async def test_discovery_update_unchanged_light(opp, mqtt_mock, caplog):
    ...
    with patch(
        "openpeerpower.components.mqtt.light.schema_json.MqttLightJson.discovery_update"
    ) as discovery_update:
...
async def test_entity_debug_info_message(opp, mqtt_mock):
    ...
    await help_test_entity_debug_info_message(
        opp, mqtt_mock, light.DOMAIN, DEFAULT_CONFIG, payload='{"state":"ON"}'
    )
