...
async def test_legacy_rgb_white_light(opp, mqtt_mock):
...
async def test_legacy_controlling_state_via_topic(opp, mqtt_mock):
...
async def test_controlling_state_via_topic(opp, mqtt_mock):
    ...
    async_fire_mqtt_message(opp, "test_light_rgb/rgbw/status", "80,40,20,10")
    light_state = opp.states.get("light.test")
    assert light_state.attributes.get("rgbw_color") == (80, 40, 20, 10)
    assert light_state.attributes.get(light.ATTR_COLOR_MODE) == "rgbw"
    assert light_state.attributes.get(light.ATTR_SUPPORTED_COLOR_MODES) == color_modes

    async_fire_mqtt_message(opp, "test_light_rgb/rgbww/status", "80,40,20,10,8")
    light_state = opp.states.get("light.test")
...
async def test_legacy_invalid_state_via_topic(opp, mqtt_mock, caplog):
...
async def test_invalid_state_via_topic(opp, mqtt_mock, caplog):
    ...
    async_fire_mqtt_message(opp, "test_light_rgb/status", "1")
    async_fire_mqtt_message(opp, "test_light_rgb/color_mode/status", "rgb")
    async_fire_mqtt_message(opp, "test_light_rgb/rgb/status", "255,255,255")
    async_fire_mqtt_message(opp, "test_light_rgb/brightness/status", "255")
    async_fire_mqtt_message(opp, "test_light_rgb/effect/status", "none")
    ...
    async_fire_mqtt_message(opp, "test_light_rgb/color_mode/status", "")
    ...
    async_fire_mqtt_message(opp, "test_light_rgb/rgbw/status", "255,255,255,1")
    async_fire_mqtt_message(opp, "test_light_rgb/color_mode/status", "rgbw")
    async_fire_mqtt_message(opp, "test_light_rgb/rgbw/status", "")
    assert "Ignoring empty rgbw message" in caplog.text
    light_state = opp.states.get("light.test")
    assert light_state.attributes.get("rgbw_color") == (255, 255, 255, 1)

    async_fire_mqtt_message(opp, "test_light_rgb/rgbww/status", "255,255,255,1,2")
    async_fire_mqtt_message(opp, "test_light_rgb/color_mode/status", "rgbww")
    async_fire_mqtt_message(opp, "test_light_rgb/rgbww/status", "")
    assert "Ignoring empty rgbww message" in caplog.text
    light_state = opp.states.get("light.test")
    assert light_state.attributes.get("rgbww_color") == (255, 255, 255, 1, 2)

    async_fire_mqtt_message(opp, "test_light_rgb/color_temp/status", "153")
    async_fire_mqtt_message(opp, "test_light_rgb/color_mode/status", "color_temp")
...
async def test_legacy_white_value_controlling_scale(opp, mqtt_mock):
...
async def test_legacy_controlling_state_via_topic_with_templates(opp, mqtt_mock):
...
async def test_controlling_state_via_topic_with_templates(opp, mqtt_mock):
    ...
    async_fire_mqtt_message(
        opp, "test_light_rgb/rgbw/status", '{"hello": [1, 2, 3, 4]}'
    )
    state = opp.states.get("light.test")
    assert state.state == STATE_ON
    ...
    async_fire_mqtt_message(
        opp, "test_light_rgb/rgbww/status", '{"hello": [1, 2, 3, 4, 5]}'
    )
    state = opp.states.get("light.test")
    assert state.state == STATE_ON
...
async def test_legacy_sending_mqtt_commands_and_optimistic(opp, mqtt_mock):
...
async def test_sending_mqtt_commands_and_optimistic(opp, mqtt_mock):
    ...
    await common.async_turn_on(opp, "light.test", effect="colorloop")
    mqtt_mock.async_publish.assert_has_calls(
        [
            call("test_light_rgb/set", "on", 2, False),
    ...
    await common.async_turn_on(
        opp, "light.test", brightness=20, rgbw_color=[80, 40, 20, 10]
    )
    mqtt_mock.async_publish.assert_has_calls(
        [
            call("test_light_rgb/set", "on", 2, False),
    ...
    await common.async_turn_on(
        opp, "light.test", brightness=40, rgbww_color=[80, 40, 20, 10, 8]
    )
    mqtt_mock.async_publish.assert_has_calls(
        [
            call("test_light_rgb/set", "on", 2, False),
    ...
    await common.async_turn_on(opp, "light.test", brightness=60, xy_color=[0.2, 0.3])
    mqtt_mock.async_publish.assert_has_calls(
        [
            call("test_light_rgb/set", "on", 2, False),
    ...
    await common.async_turn_on(opp, "light.test", color_temp=125)
    mqtt_mock.async_publish.assert_has_calls(
        [
            call("test_light_rgb/color_temp/set", "125", 2, False),
        ],
...
async def test_sending_mqtt_rgbw_command_with_template(opp, mqtt_mock):
    ...
    await common.async_turn_on(opp, "light.test", rgbw_color=[255, 128, 64, 32])
...
async def test_sending_mqtt_rgbww_command_with_template(opp, mqtt_mock):
    ...
    await common.async_turn_on(opp, "light.test", rgbww_color=[255, 128, 64, 32, 16])
...
async def test_legacy_on_command_rgb(opp, mqtt_mock):
...
async def test_on_command_rgbw(opp, mqtt_mock):
    ...
    # Ensure color gets scaled with brightness.
    await common.async_turn_on(opp, "light.test", rgbw_color=[255, 128, 0, 16])
...
async def test_on_command_rgbww(opp, mqtt_mock):
    ...
    # Ensure color gets scaled with brightness.
    await common.async_turn_on(opp, "light.test", rgbww_color=[255, 128, 0, 16, 32])
...
async def test_on_command_rgbw_template(opp, mqtt_mock):
...
async def test_on_command_rgbww_template(opp, mqtt_mock):
...
async def test_explicit_color_mode(opp, mqtt_mock):
    ...
    async_fire_mqtt_message(opp, "test_light_rgb/color_mode/status", "hs")
    light_state = opp.states.get("light.test")
    assert light_state.attributes.get("hs_color") == (200, 50)
    assert light_state.attributes.get(light.ATTR_COLOR_MODE) == "hs"
    assert light_state.attributes.get(light.ATTR_SUPPORTED_COLOR_MODES) == color_modes

    async_fire_mqtt_message(opp, "test_light_rgb/color_mode/status", "xy")
    light_state = opp.states.get("light.test")
    assert light_state.attributes.get("xy_color") == (0.675, 0.322)
    assert light_state.attributes.get(light.ATTR_COLOR_MODE) == "xy"
    assert light_state.attributes.get(light.ATTR_SUPPORTED_COLOR_MODES) == color_modes
...
async def test_explicit_color_mode_templated(opp, mqtt_mock):
    ...
    async_fire_mqtt_message(
        opp, "test_light_rgb/color_mode/status", '{"color_mode":"color_temp"}'
    )
    light_state = opp.states.get("light.test")
    assert light_state.attributes.get(light.ATTR_COLOR_MODE) == "color_temp"
    assert light_state.attributes.get(light.ATTR_SUPPORTED_COLOR_MODES) == color_modes
    ...
    async_fire_mqtt_message(
        opp, "test_light_rgb/color_mode/status", '{"color_mode":"hs"}'
    )
    light_state = opp.states.get("light.test")
    assert light_state.attributes.get("hs_color") == (200, 50)
    assert light_state.attributes.get(light.ATTR_COLOR_MODE) == "hs"
