...
async def dsmr_connection_fixture(opp):
    ...
    with patch(
        "openpeerpower.components.dsmr.sensor.create_dsmr_reader", connection_factory
    ), patch(
        "openpeerpower.components.dsmr.sensor.create_tcp_dsmr_reader",
        connection_factory,
    ):
...
async def dsmr_connection_send_validate_fixture(opp):
    ...
    with patch(
        ...
        "openpeerpower.components.dsmr.config_flow.create_tcp_dsmr_reader",
        connection_factory,
    ):
        yield (connection_factory, transport, protocol)
