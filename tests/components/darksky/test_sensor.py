...
class TestDarkSkySetup(unittest.TestCase):
    ...
    @patch(
        "openpeerpower.components.darksky.sensor.forecastio.load_forecast",
        new=load_forecastMock,
    )
    def test_setup_with_config(self):
        """Test the platform setup with configuration."""
        setup_component(self.opp, "sensor", VALID_CONFIG_MINIMAL)
        self.opp.block_till_done()

        state = self.opp.states.get("sensor.dark_sky_summary")
        assert state is not None

    def test_setup_with_invalid_config(self):
        """Test the platform setup with invalid configuration."""
        setup_component(self.opp, "sensor", INVALID_CONFIG_MINIMAL)
        self.opp.block_till_done()
    ...
    def test_setup_with_language_config(self):
        ...
        setup_component(self.opp, "sensor", VALID_CONFIG_LANG_DE)
        self.opp.block_till_done()
    ...
    def test_setup_with_invalid_language_config(self):
        """Test the platform setup with language configuration."""
        setup_component(self.opp, "sensor", INVALID_CONFIG_LANG)
        self.opp.block_till_done()
    ...
    def test_setup_bad_api_key(self, mock_get_forecast):
        ...
        response = darksky.setup_platform(
            self.opp, VALID_CONFIG_MINIMAL["sensor"], MagicMock()
        )
        assert not response
    ...
    def test_setup_with_alerts_config(self):
        ...
        setup_component(self.opp, "sensor", VALID_CONFIG_ALERTS)
        self.opp.block_till_done()

        state = self.opp.states.get("sensor.dark_sky_alerts")
    ...
    def test_setup(self, mock_req, mock_get_forecast):
        ...
        assert len(self.opp.states.entity_ids()) == 13
        ...
        state = self.opp.states.get("sensor.dark_sky_daytime_high_temperature_1d")
        assert state is not None
