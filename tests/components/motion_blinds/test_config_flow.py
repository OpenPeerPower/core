...
from openpeerpower.components.motion_blinds.config_flow import DEFAULT_GATEWAY_NAME
from openpeerpower.components.motion_blinds.const import DOMAIN
...
def motion_blinds_connect_fixture():
    ...
    with patch(
        ...
        "openpeerpower.components.motion_blinds.gateway.MotionGateway.Update",
        return_value=True,
    ), patch(
        "openpeerpower.components.motion_blinds.gateway.MotionGateway.device_list",
        TEST_DEVICE_LIST,
    ), patch(
        "openpeerpower.components.motion_blinds.config_flow.MotionDiscovery.discover",
        return_value=TEST_DISCOVERY_1,
    ), patch(
        "openpeerpower.components.motion_blinds.async_setup_entry", return_value=True
    ):
        yield
