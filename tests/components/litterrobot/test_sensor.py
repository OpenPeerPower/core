...
from openpeerpower.components.litterrobot.sensor import LitterRobotSleepTimeSensor
from openpeerpower.components.sensor import DOMAIN as PLATFORM_DOMAIN
from openpeerpower.const import DEVICE_CLASS_TIMESTAMP, PERCENTAGE
...
async def test_waste_drawer_sensor(opp, mock_account):
    """Tests the waste drawer sensor entity was set up."""
    await setup_integration(opp, mock_account, PLATFORM_DOMAIN)

    sensor = opp.states.get(WASTE_DRAWER_ENTITY_ID)
    assert sensor
    assert sensor.state == "50.0"
    assert sensor.attributes["unit_of_measurement"] == PERCENTAGE


async def test_sleep_time_sensor_with_none_state(opp):
...
async def test_gauge_icon():
    ...
    from openpeerpower.components.litterrobot.sensor import icon_for_gauge_level
