...
from openpeerpower.components.elgato.const import DOMAIN, SERVICE_IDENTIFY
...
async def test_light_state_color(
    ...
    """Test the creation and values of the Elgato Lights in temperature mode."""
    await init_integration(opp, aioclient_mock, color=True, mode_color=True)
...
async def test_light_change_state_temperature(
    ...
    await init_integration(opp, aioclient_mock, color=True, mode_color=False)
...
async def test_light_change_state_color(
    ...
    await init_integration(opp, aioclient_mock, color=True)
...
async def test_light_unavailable(
    service: str, opp: OpenPeerPower, aioclient_mock: AiohttpClientMocker
) -> None:
...
async def test_light_identify(
    ...
    with patch(
        "openpeerpower.components.elgato.light.Elgato.identify",
        return_value=mock_coro(),
    ) as mock_identify:
        await opp.services.async_call(
            DOMAIN,
