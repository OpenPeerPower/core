...
from openpeerpower.components.august.const import CONF_LOGIN_METHOD, DOMAIN
...
async def _mock_setup_august(
    opp, api_instance, pubnub_mock, authenticate_mock, api_mock
):
    ...
    with patch("openpeerpower.components.august.async_create_pubnub"), patch(
        "openpeerpower.components.august.AugustPubNub", return_value=pubnub_mock
    ):
...
async def _create_august_with_devices(  # noqa: C901
    opp, devices, api_call_side_effects=None, activities=None, pubnub=None
):
    ...
    return await _mock_setup_august_with_api_side_effects(
        opp, api_call_side_effects, pubnub
    )


async def _mock_setup_august_with_api_side_effects(opp, api_call_side_effects, pubnub):
    ...
    return await _mock_setup_august(opp, api_instance, pubnub)
