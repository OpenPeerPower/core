...
from openpeerpower.components.modbus import number
...
async def test_config_modbus(opp, caplog, do_config, mock_pymodbus):
...
async def test_pb_service_write(opp, do_write, caplog, mock_modbus):
    ...
    await opp.services.async_call(DOMAIN, do_write[SERVICE], data, blocking=True)
...
async def _read_helper(opp, do_group, do_type, do_return, do_exception, mock_pymodbus):
    config = {
        DOMAIN: [
            {
                CONF_TYPE: "tcp",
...
async def test_pb_read_value(
    opp, caplog, do_type, do_return, do_exception, do_expect, mock_pymodbus
):
    ...
    await _read_helper(
        opp, CONF_SENSORS, do_type, do_return, do_exception, mock_pymodbus
    )

    # Check state
    entity_id = f"{SENSOR_DOMAIN}.{TEST_SENSOR_NAME}"
    assert opp.states.get(entity_id).state
...
async def test_pb_read_state(
    ...
    # to make a proper blackbox test.
    await _read_helper(
        opp, CONF_BINARY_SENSORS, do_type, do_return, do_exception, mock_pymodbus
    )
...
async def test_pymodbus_constructor_fail(opp, caplog):
    ...
    with mock.patch(
        "openpeerpower.components.modbus.modbus.ModbusTcpClient"
    ) as mock_pb:
        caplog.set_level(logging.ERROR)
...
async def test_pymodbus_connect_fail(opp, caplog, mock_pymodbus):
    """Run test for failing pymodbus constructor."""
    config = {
        DOMAIN: [
            {
...
async def test_delay(opp, mock_pymodbus):
