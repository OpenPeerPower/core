...
from openpeerpower.components.modbus.validators import (
    number_validator,
...
@pytest.fixture
async def mock_modbus_with_pymodbus(opp, caplog, do_config, mock_pymodbus):
...
async def test_config_modbus(opp, caplog, mock_modbus_with_pymodbus):
...
async def test_pb_service_write(
    opp, do_write, do_return, caplog, mock_modbus_with_pymodbus
):
...
async def test_pb_read(
    opp, do_domain, do_expect_state, do_expect_value, caplog, mock_modbus_read_pymodbus
):
