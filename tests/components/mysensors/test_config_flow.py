...
from openpeerpower.components.mysensors.const import (
    CONF_BAUD_RATE,
    CONF_DEVICE,
    CONF_GATEWAY_TYPE,
...
async def test_config_serial(opp: OpenPeerPowerType):
    ...
    with patch(  # mock is_serial_port because otherwise the test will be platform dependent (/dev/ttyACMx vs COMx)
        "openpeerpower.components.mysensors.config_flow.is_serial_port",
        return_value=True,
    ), patch(
...
async def test_fail_to_connect(opp: OpenPeerPowerType):
    ...
    with patch(
        "openpeerpower.components.mysensors.config_flow.try_connect", return_value=False
    ), patch(
        "openpeerpower.components.mysensors.async_setup", return_value=True
    ) as mock_setup, patch(
        "openpeerpower.components.mysensors.async_setup_entry",
