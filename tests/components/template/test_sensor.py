...
from openpeerpower.bootstrap import async_from_config_dict
...
async def test_template_legacy(opp):
    """Test template."""
...
async def test_trigger_entity(opp):
    ...
    state = opp.states.get("sensor.hello_name")
    assert state is not None
    assert state.state == STATE_UNKNOWN

    state = opp.states.get("sensor.bare_minimum")
    assert state is not None
    assert state.state == STATE_UNKNOWN
    ...
    state = opp.states.get("sensor.via_list")
...
async def test_trigger_entity_render_error(opp):
    ...
    state = opp.states.get("sensor.hello")
    assert state is not None
    assert state.state == STATE_UNKNOWN
...
async def test_trigger_not_allowed_platform_config(opp, caplog):
