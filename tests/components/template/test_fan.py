...
async def test_templates_with_entities(opp, calls):
    ...
    opp.states.async_set(_STATE_INPUT_BOOLEAN, False)
    await opp.async_block_till_done()
    _verify(opp, STATE_OFF, None, 0, True, DIRECTION_FORWARD, None)
...
async def test_templates_with_entities_and_invalid_percentage(opp, calls):
    ...
    _verify(opp, STATE_ON, SPEED_OFF, 0, None, None, None)
...
async def test_increase_decrease_speed(opp, calls):
    ...
    await _register_components(opp, speed_count=3)
...
async def test_increase_decrease_speed_default_speed_count(opp, calls):
    """Test set valid increase and decrease speed."""
    ...
    await common.async_decrease_speed(opp, _TEST_FAN)
    assert int(float(opp.states.get(_PERCENTAGE_INPUT_NUMBER).state)) == 99

    _verify(opp, STATE_ON, SPEED_HIGH, 99, None, None, None)

    # Set fan's percentage speed to 98
    await common.async_decrease_speed(opp, _TEST_FAN)
    assert int(float(opp.states.get(_PERCENTAGE_INPUT_NUMBER).state)) == 98

    _verify(opp, STATE_ON, SPEED_HIGH, 98, None, None, None)
...
async def _register_components(
    opp, speed_list=None, preset_modes=None, speed_count=None
):
...
async def test_implemented_percentage(opp, speed_count, percentage_step):
    ...
    state = opp.states.get("fan.mechanical_ventilation")
    attributes = state.attributes
    assert attributes["percentage_step"] == percentage_step


async def test_implemented_preset_mode(opp):
...
async def test_implemented_speed(opp):
