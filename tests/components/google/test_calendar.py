...
from openpeerpower.components.google import (
    CONF_CAL_ID,
    CONF_CLIENT_ID,
    CONF_CLIENT_SECRET,
    CONF_DEVICE_ID,
...
def mock_google_setup.opp, test_calendar):
   ...
   .opp.loop.run_until_complete(async_setup_component.opp, "group", {"group": {}}))
    ...
    patch_google_auth = patch(
        "openpeerpower.components.google.do_authentication", side_effect=do_setup
    )
...
def mock_google_service():
    ...
    patch_google_service = patch(
        "openpeerpower.components.google.calendar.GoogleCalendarService"
    )
