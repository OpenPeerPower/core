...
from openpeerpower.const import ATTR_ATTRIBUTION, ATTR_FRIENDLY_NAME
...
async def _setup(opp: OpenPeerPower, config: dict[str, Any]) -> State:
    ...
    with patch(
        ...
        for entity_name in ("hourly", "nowcast"):
            _enable_entity(opp, f"weather.climacell_{entity_name}")
        await opp.async_block_till_done()
        assert len(opp.states.async_entity_ids(WEATHER_DOMAIN)) == 3

    return opp.states.get("weather.climacell_daily")
...
async def test_v3_weather(
    ...
    weather_state = await _setup(opp, API_V3_ENTRY_DATA)
...
async def test_v4_weather(
    ...
    weather_state = await _setup(opp, API_V4_ENTRY_DATA)
    assert weather_state.state == ATTR_CONDITION_SUNNY
    assert weather_state.attributes[ATTR_ATTRIBUTION] == ATTRIBUTION
    assert weather_state.attributes[ATTR_FORECAST] == [
        {
