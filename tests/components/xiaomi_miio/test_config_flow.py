...
async def test_config_flow_step_device_manual_model_succes(opp):
...
async def test_config_flow_vacuum_success(opp):
    """Test a successful config flow for a vacuum."""
    test_vacuum_model = const.MODELS_VACUUM[0]
    await config_flow_device_success(opp, test_vacuum_model)


async def test_zeroconf_vacuum_success(opp):
    """Test a successful zeroconf discovery of a vacuum."""
    test_vacuum_model = const.MODELS_VACUUM[0]
    test_zeroconf_name = const.MODELS_VACUUM[0].replace(".", "-")
    await zeroconf_device_success(opp, test_zeroconf_name, test_vacuum_model)
