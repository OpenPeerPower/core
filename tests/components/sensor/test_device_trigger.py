...
async def test_if_fires_on_state_above(opp, calls, enable_custom_integrations):
    """Test for value triggers firing."""
    platform = getattr(opp.components, f"test.{DOMAIN}")
    platform.init()
    assert await async_setup_component(opp, DOMAIN, {DOMAIN: {CONF_PLATFORM: "test"}})
...
async def test_if_fires_on_state_below(opp, calls, enable_custom_integrations):
    """Test for value triggers firing."""
    platform = getattr(opp.components, f"test.{DOMAIN}")
    platform.init()
    assert await async_setup_component(opp, DOMAIN, {DOMAIN: {CONF_PLATFORM: "test"}})
...
async def test_if_fires_on_state_between(opp, calls, enable_custom_integrations):
    """Test for value triggers firing."""
    platform = getattr(opp.components, f"test.{DOMAIN}")
    platform.init()
    assert await async_setup_component(opp, DOMAIN, {DOMAIN: {CONF_PLATFORM: "test"}})
