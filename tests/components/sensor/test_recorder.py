...
def test_compile_hourly_energy_statistics(opp_recorder):
    ...
    zero, four, eight, states = record_energy_states(
        opp, sns1_attr, sns2_attr, sns3_attr
    )
    hist = history.get_significant_states(
        opp, zero - timedelta.resolution, eight + timedelta.resolution
    )
...
def test_compile_hourly_energy_statistics2(opp_recorder):
    """Test compiling hourly statistics."""
    opp = opp_recorder()
    recorder = opp.data[DATA_INSTANCE]
    setup_component(opp, "sensor", {})
...
def test_compile_hourly_statistics_partially_unavailable(opp_recorder):
...
def record_energy_states(opp, _sns1_attr, _sns2_attr, _sns3_attr):
    """Record some test states.
    ...
    with patch("openpeerpower.components.recorder.dt_util.utcnow", return_value=zero):
    ...
    with patch("openpeerpower.components.recorder.dt_util.utcnow", return_value=four):
    ...
    with patch("openpeerpower.components.recorder.dt_util.utcnow", return_value=five):
    ...
    with patch("openpeerpower.components.recorder.dt_util.utcnow", return_value=six):
    ...
    with patch("openpeerpower.components.recorder.dt_util.utcnow", return_value=seven):
    ...
    with patch("openpeerpower.components.recorder.dt_util.utcnow", return_value=eight):
...
def record_states_partially_unavailable(opp):
    """Record some test states.
