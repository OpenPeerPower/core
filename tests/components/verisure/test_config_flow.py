...
from openpeerpower.components.dhcp import MAC_ADDRESS
from openpeerpower.components.verisure.const import (
    CONF_GIID,
...
async def test_full_user_flow_single_installation(opp: OpenPeerPower) -> None:
    ...
    with patch(
        "openpeerpower.components.verisure.config_flow.Verisure",
    ) as mock_verisure, patch(
        "openpeerpower.components.verisure.async_setup_entry",
        return_value=True,
    ) as mock_setup_entry:
...
async def test_full_user_flow_multiple_installations(opp: OpenPeerPower) -> None:
...
async def test_invalid_login(opp: OpenPeerPower) -> None:
    ...
    with patch(
        "openpeerpower.components.verisure.config_flow.Verisure.login",
...
async def test_unknown_error(opp: OpenPeerPower) -> None:
    """Test a flow with an invalid Verisure My Pages login."""
    result = await opp.config_entries.flow.async_init(
        DOMAIN, context={"source": config_entries.SOURCE_USER}
    )
...
async def test_dhcp(opp: OpenPeerPower) -> None:
...
async def test_reauth_flow_invalid_login(opp: OpenPeerPower) -> None:
    """Test a reauthentication flow."""
    entry = MockConfigEntry(
        domain=DOMAIN,
        unique_id="12345",
...
async def test_reauth_flow_unknown_error(opp: OpenPeerPower) -> None:
...
async def test_options_flow(
    opp: OpenPeerPower, input: dict[str, int | str], output: dict[str, int | str]
) -> None:
...
async def test_options_flow_code_format_mismatch(opp: OpenPeerPower) -> None:
