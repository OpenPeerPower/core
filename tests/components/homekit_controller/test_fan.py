...
async def test_turn_on_off_without_rotation_speed(opp, utcnow):
    """Test that we can turn a fan on."""
    helper = await setup_test_component(
        opp, create_fanv2_service_without_rotation_speed
    )
...
async def test_v2_set_percentage_with_min_step(opp, utcnow):
    """Test that we set fan speed by percentage."""
    helper = await setup_test_component(opp, create_fanv2_service_with_min_step)
