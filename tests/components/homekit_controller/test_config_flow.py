...
def get_device_discovery_info(device, upper_case_props=False, missing_csharp=False):
    """Turn a aiohomekit format zeroconf entry into a openpeerpower one."""
    record = device.info
    result = {
        "host": record["address"],
        "port": record["port"],
...
async def test_pair_abort_errors_on_finish(opp, controller, exception, expected):
    ...
    # and then OPP to show a pairing form
...
async def test_user_pairing_with_insecure_setup_code(opp, controller):
    """Test user initiated disovers devices."""
    device = setup_mock_accessory(controller)
