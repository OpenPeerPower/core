...
async def test_create_account(opp, client, enable_custom_integrations):
...
async def test_get_progress_index(opp, opp_ws_client):
    ...
    with patch.dict(HANDLERS, {"test": TestFlow}):
        form = await opp.config_entries.flow.async_init(
            "test", context={"source": core_ce.SOURCE_OPPIO}
        )
    ...
    assert response["result"] == [
        {
            ...
            "handler": "test",
            "step_id": "account",
            "context": {"source": core_ce.SOURCE_OPPIO},
        }
    ]
...
async def test_update_prefrences(opp, opp_ws_client):
