...
from openpeerpower.components.foscam import config_flow
...
async def test_user_valid.opp):
    ...
    with patch(
        ...
        "openpeerpower.components.foscam.async_setup", return_value=True
    ) as mock_setup, patch(
        "openpeerpower.components.foscam.async_setup_entry",
        return_value=True,
    ) as mock_setup_entry:
