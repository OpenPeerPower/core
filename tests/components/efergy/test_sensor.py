...
async def test_single_sensor_readings(opp, requests_mock):
    ...
    assert opp.states.get("sensor.energy_consumed").state == "38.21"
    assert opp.states.get("sensor.energy_usage").state == "1580"
    assert opp.states.get("sensor.energy_budget").state == "ok"
    assert opp.states.get("sensor.energy_cost").state == "5.27"
    assert opp.states.get("sensor.efergy_728386").state == "1628"
...
async def test_multi_sensor_readings(opp, requests_mock):
    ...
    assert opp.states.get("sensor.efergy_728386").state == "218"
    assert opp.states.get("sensor.efergy_0").state == "1808"
    assert opp.states.get("sensor.efergy_728387").state == "312"
