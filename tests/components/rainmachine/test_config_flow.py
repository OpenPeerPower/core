...
from openpeerpower.components.rainmachine import CONF_ZONE_RUN_TIME, DOMAIN
...
async def test_step_homekit_zeroconf_ip_already_exists(opp, source):
...
async def test_step_homekit_zeroconf_ip_change(opp, source):
...
async def test_step_homekit_zeroconf_new_controller_when_some_exist(opp, source):
...
async def test_discovery_by_homekit_and_zeroconf_same_time(opp):
