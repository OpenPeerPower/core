...
from openpeerpower.components.hue import bridge, errors
from openpeerpower.components.hue.const import (
...
def mock_subscribe_events():
    ...
    with patch(
        "openpeerpower.components.hue.bridge.HueBridge._subscribe_events"
    ) as mock:
        yield mock


async def test_bridge_setup(opp, mock_subscribe_events):
...
async def test_reset_unloads_entry_if_setup(opp, mock_subscribe_events):
...
async def test_event_updates(opp, caplog):
    """Test calling reset while the entry has been setup."""
