...
from openpeerpower.components.netatmo import sensor
from openpeerpower.components.netatmo.sensor import MODULE_TYPE_WIND
from openpeerpower.helpers import entity_registry as er

from .common import TEST_TIME, selected_platforms


async def test_weather_sensor(opp, config_entry, netatmo_auth):
    ...
    assert opp.states.get(f"{prefix}temperature").state == "24.6"
    assert opp.states.get(f"{prefix}humidity").state == "36"
    assert opp.states.get(f"{prefix}co2").state == "749"
    assert opp.states.get(f"{prefix}pressure").state == "1017.3"


async def test_public_weather_sensor(opp, config_entry, netatmo_auth):
    ...
    assert opp.states.get(f"{prefix}temperature").state == "27.4"
    assert opp.states.get(f"{prefix}humidity").state == "76"
    assert opp.states.get(f"{prefix}pressure").state == "1014.4"

    prefix = "sensor.netatmo_home_avg_"

    assert opp.states.get(f"{prefix}temperature").state == "22.7"
    assert opp.states.get(f"{prefix}humidity").state == "63.2"
    assert opp.states.get(f"{prefix}pressure").state == "1010.3"

    entities_before_change = len(opp.states.async_all())
    ...
    assert len(opp.states.async_all()) == entities_before_change
    assert opp.states.get(f"{prefix}temperature").state == "27.4"
...
async def test_weather_sensor_enabling(
    opp, config_entry, uid, name, expected, netatmo_auth
):
    """Test enabling of by default disabled sensors."""
    with patch("time.time", return_value=TEST_TIME), selected_platforms(["sensor"]):
        states_before = len(opp.states.async_all())
        assert opp.states.get(f"sensor.{name}") is None
        ...
        assert len(opp.states.async_all()) > states_before
        assert opp.states.get(f"sensor.{name}").state == expected
