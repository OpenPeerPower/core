...
from openpeerpower.components.onvif import config_flow
...
async def setup_onvif_integration(
    ...
    with patch(
        ...
        "openpeerpower.components.onvif.config_flow.wsdiscovery"
    ) as mock_discovery, patch(
        "openpeerpower.components.onvif.ONVIFDevice"
    ) as mock_device:
...
async def test_flow_discovered_devices(opp):
    ...
    with patch(
        ...
        with patch(
            "openpeerpower.components.onvif.async_setup", return_value=True
        ) as mock_setup, patch(
            "openpeerpower.components.onvif.async_setup_entry", return_value=True
        ) as mock_setup_entry:
            result = await opp.config_entries.flow.async_configure(
                result["flow_id"],
                user_input={
...
async def test_flow_import_not_implemented(opp):
