...
from openpeerpower.config_entries import SOURCE_DHCP, SOURCE_USER, SOURCE_ZEROCONF
...
async def test_duplicate_error(opp, ping_client):
    ...
    MockConfigEntry(domain=DOMAIN, unique_id="guardian_3456", data=conf).add_to_opp(opp)
...
async def test_step_dhcp(opp, ping_client):
    """Test the dhcp step."""
    dhcp_data = {
...
async def test_step_dhcp_already_in_progress(opp):
    """Test the zeroconf step aborting because it's already in progress."""
    dhcp_data = {
        IP_ADDRESS: "192.168.1.100",
        HOSTNAME: "GVC1-ABCD.local.",
