...
def vizio_get_unique_id_fixture():
    ...
    with patch(
        "openpeerpower.components.vizio.config_flow.VizioAsync.get_unique_id",
...
def vizio_connect_fixture():
    ...
    with patch(
        "openpeerpower.components.vizio.config_flow.VizioAsync.validate_op.config",
...
def vizio_complete_pairing_fixture():
    ...
    with patch(
        "openpeerpower.components.vizio.config_flow.VizioAsync.start_pair",
        return_value=MockStartPairingResponse(CH_TYPE, RESPONSE_TOKEN),
    ), patch(
        "openpeerpower.components.vizio.config_flow.VizioAsync.pair",
...
def vizio_bypass_setup_fixture():
    ...
    with patch("openpeerpower.components.vizio.async_setup_entry", return_value=True):
        yield
...
def vizio_bypass_update_fixture():
    ...
    with patch(
        ...
        return_value=True,
    ), patch("openpeerpower.components.vizio.media_player.VizioDevice.async_update"):
        yield
...
def vizio_guess_device_type_fixture():
    ...
    with patch(
        "openpeerpower.components.vizio.config_flow.async_guess_device_type",
...
def vizio_update_fixture():
    ...
    with patch(
        ...
        "openpeerpower.components.vizio.media_player.VizioAsync.get_current_input",
        return_value=CURRENT_INPUT,
    ), patch(
        "openpeerpower.components.vizio.media_player.VizioAsync.get_inputs_list",
        return_value=get_mock_inputs(INPUT_LIST),
    ), patch(
        "openpeerpower.components.vizio.media_player.VizioAsync.get_power_state",
        return_value=True,
    ), patch(
        "openpeerpower.components.vizio.media_player.VizioAsync.get_model_name",
        return_value=MODEL,
    ), patch(
        "openpeerpower.components.vizio.media_player.VizioAsync.get_version",
...
def vizio_hostname_check():
    ...
    with patch(
        "openpeerpower.components.vizio.config_flow.socket.gethostbyname",
