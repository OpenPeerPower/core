...
def mock_remote_fixture(camera_info):
    ...
    with patch("openpeerpower.components.uvc.camera.nvr.UVCRemote") as mock_remote:
...
def camera_v320_fixture():
    ...
    with patch(
        "openpeerpower.components.uvc.camera.uvc_camera.UVCCameraClientV320"
    ) as camera:
...
def camera_v313_fixture():
    """Mock the v320 camera."""
    with patch(
        "openpeerpower.components.uvc.camera.uvc_camera.UVCCameraClient"
    ) as camera:
        camera.return_value.get_snapshot.return_value = "test_image"
        yield camera
...
async def test_setup_partial_config(opp, mock_remote):
...
async def test_setup_incomplete_config(opp, mock_remote, config):
    """Test the setup with incomplete or invalid configuration."""
    assert await async_setup_component(opp, "camera", config)
    await opp.async_block_till_done()
...
async def test_setup_nvr_errors_during_indexing(opp, mock_remote, error, ready_states):
    ...
    async_fire_time_changed(opp, now + timedelta(seconds=31))
    await opp.async_block_till_done()
...
async def test_setup_nvr_errors_during_initialization(
    opp, mock_remote, error, ready_states
):
...
async def test_properties(opp, mock_remote):
    """Test the properties."""
    config = {"platform": "uvc", "nvr": "foo", "key": "secret"}
    assert await async_setup_component(opp, "camera", {"camera": config})
    await opp.async_block_till_done()
...
async def test_motion_recording_mode_properties(opp, mock_remote):
    """Test the properties."""
    config = {"platform": "uvc", "nvr": "foo", "key": "secret"}
    now = utcnow()
    assert await async_setup_component(opp, "camera", {"camera": config})
    ...
    async_fire_time_changed(opp, now + timedelta(seconds=91))
    await opp.async_block_till_done()
    ...
    async_fire_time_changed(opp, now + timedelta(seconds=121))
    await opp.async_block_till_done()
...
async def test_stream(opp, mock_remote):
    """Test the RTSP stream URI."""
    config = {"platform": "uvc", "nvr": "foo", "key": "secret"}
    assert await async_setup_component(opp, "camera", {"camera": config})
    await opp.async_block_till_done()

    stream_source = await async_get_stream_source(opp, "camera.front")

    assert stream_source == "rtsp://foo:7447/uuid_rtspchannel_0"


async def test_login(opp, mock_remote, camera_v320):
    """Test the login."""
    config = {"platform": "uvc", "nvr": "foo", "key": "secret"}
    assert await async_setup_component(opp, "camera", {"camera": config})
    await opp.async_block_till_done()

    image = await async_get_image(opp, "camera.front")
...
async def test_login_v31x(opp, mock_remote, camera_v313):
...
async def test_login_tries_both_addrs_and_caches(opp, mock_remote, camera_v320, error):
...
async def test_login_fails_both_properly(opp, mock_remote, camera_v320):
    ...
    with pytest.raises(OpenPeerPowerError):
        await async_get_image(opp, "camera.front")
...
@pytest.mark.parametrize(
    ...
    [
        (camera.CameraConnectError, OpenPeerPowerError, 1),
        (camera.CameraAuthError, camera.CameraAuthError, 2),
    ],
)
async def test_camera_image_error(
    opp, mock_remote, camera_v320, source_error, raised_error, snapshot_calls
):
...
async def test_enable_disable_motion_detection(opp, mock_remote, camera_info):
