...
async def test_options_migration(opp):
    ...
    with patch(
        "openpeerpower.components.opentherm_gw.OpenThermGatewayDevice.connect_and_subscribe",
        return_value=True,
    ), patch("openpeerpower.components.opentherm_gw.async_setup", return_value=True):
...
async def test_options_form(opp):
    ...
    with patch(
        "openpeerpower.components.opentherm_gw.async_setup", return_value=True
    ), patch(
        "openpeerpower.components.opentherm_gw.async_setup_entry", return_value=True
    ):
        await opp.config_entries.async_setup(entry.entry_id)
        await opp.async_block_till_done()
