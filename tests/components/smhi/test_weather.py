...
from openpeerpower.components.smhi.const import (
    ATTR_SMHI_CLOUDINESS,
    ATTR_SMHI_THUNDER_PROBABILITY,
    ATTR_SMHI_WIND_GUST_SPEED,
)
from openpeerpower.components.smhi.weather import CONDITION_CLASSES, RETRY_TIMEOUT
...
async def test_properties_no_data(opp: OpenPeerPower) -> None:
    """Test properties when no API data available."""
    entry = MockConfigEntry(domain="smhi", data=TEST_CONFIG)
    entry.add_to_opp(opp)

    with patch(
        "openpeerpower.components.smhi.weather.Smhi.async_get_forecast",
...
async def test_properties_unknown_symbol(opp: OpenPeerPower) -> None:
...
async def test_refresh_weather_forecast_retry(
    opp: OpenPeerPower, error: Exception
) -> None:
