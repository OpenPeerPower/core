...
from openpeerpower.components.unifi.config_flow import async_discover_unifi
...
async def test_flow_works_negative_discovery(opp, aioclient_mock, mock_discovery):
...
async def test_discover_unifi_positive(opp):
    """Verify positive run of UniFi discovery."""
    with patch("socket.gethostbyname", return_value=True):
        assert await async_discover_unifi(opp)


async def test_discover_unifi_negative(opp):
    """Verify negative run of UniFi discovery."""
    with patch("socket.gethostbyname", side_effect=socket.gaierror):
        assert await async_discover_unifi(opp) is None
