...
async def test_no_entities(opp, aioclient_mock):
    """Test the update_clients function when no clients are found."""
    await setup_unifi_integration(opp, aioclient_mock)

    assert len(opp.states.async_entity_ids(TRACKER_DOMAIN)) == 0


async def test_tracked_wireless_clients(opp, aioclient_mock, mock_unifi_websocket):
    ...
    config_entry = await setup_unifi_integration(
        opp, aioclient_mock, clients_response=[client]
    )
...
async def test_tracked_devices(opp, aioclient_mock, mock_unifi_websocket):
    ...
    assert opp.states.get("device_tracker.device_1").state == STATE_HOME
    assert opp.states.get("device_tracker.device_2").state == STATE_NOT_HOME
    ...
    assert opp.states.get("device_tracker.device_1").state == STATE_HOME
    assert opp.states.get("device_tracker.device_2").state == STATE_HOME
    ...
    assert opp.states.get("device_tracker.device_1").state == STATE_NOT_HOME
    assert opp.states.get("device_tracker.device_2").state == STATE_HOME
    ...
    assert opp.states.get("device_tracker.device_1").state == STATE_UNAVAILABLE
    assert opp.states.get("device_tracker.device_2").state == STATE_HOME
...
async def test_remove_clients(opp, aioclient_mock, mock_unifi_websocket):
    ...
    assert len(opp.states.async_entity_ids(TRACKER_DOMAIN)) == 2
    assert opp.states.get("device_tracker.client_1")
    assert opp.states.get("device_tracker.client_2")
    ...
    assert len(opp.states.async_entity_ids(TRACKER_DOMAIN)) == 1
    assert not opp.states.get("device_tracker.client_1")
    assert opp.states.get("device_tracker.client_2")


async def test_controller_state_change(opp, aioclient_mock, mock_unifi_websocket):
    ...
    assert len(opp.states.async_entity_ids(TRACKER_DOMAIN)) == 2
    assert opp.states.get("device_tracker.client").state == STATE_NOT_HOME
    assert opp.states.get("device_tracker.device").state == STATE_HOME

    # Controller unavailable
    mock_unifi_websocket(state=STATE_DISCONNECTED)
    await opp.async_block_till_done()

    assert opp.states.get("device_tracker.client").state == STATE_UNAVAILABLE
    assert opp.states.get("device_tracker.device").state == STATE_UNAVAILABLE
...
async def test_option_track_clients(opp, aioclient_mock):
    ...
    assert opp.states.get("device_tracker.wireless_client")
    assert opp.states.get("device_tracker.wired_client")
    assert opp.states.get("device_tracker.device")
    ...
    assert not opp.states.get("device_tracker.wireless_client")
    assert not opp.states.get("device_tracker.wired_client")
    assert opp.states.get("device_tracker.device")
...
async def test_option_track_devices(opp, aioclient_mock):
    ...
    assert opp.states.get("device_tracker.client")
    assert not opp.states.get("device_tracker.device")
...
async def test_option_ssid_filter(opp, aioclient_mock, mock_unifi_websocket):
    ...
    assert opp.states.get("device_tracker.client").state == STATE_HOME
    assert opp.states.get("device_tracker.client_on_ssid2").state == STATE_NOT_HOME
...
async def test_option_ignore_wired_bug(opp, aioclient_mock, mock_unifi_websocket):
...
async def test_restoring_client(opp, aioclient_mock):
    ...
    assert len(opp.states.async_entity_ids(TRACKER_DOMAIN)) == 2
    assert opp.states.get("device_tracker.client")
    assert opp.states.get("device_tracker.restored")
    assert not opp.states.get("device_tracker.not_restored")
