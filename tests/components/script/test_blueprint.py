...
from openpeerpower.components.blueprint.models import Blueprint, DomainBlueprints
...
async def test_confirmable_notification(opp: OpenPeerPower) -> None:
    ...
    with patch(
        ...
        # Trigger script
        await opp.services.async_call(script.DOMAIN, "confirm", context=context)

        # Give script the time to attach the trigger.
        await asyncio.sleep(0.1)

    opp.bus.async_fire("mobile_app_notification_action", {"action": "ANYTHING_ELSE"})
    opp.bus.async_fire(
