...
async def test_alexa_config_expose_entity_prefs(opp, cloud_prefs, cloud_stub):
    ...
    conf = alexa_config.AlexaConfig(
        opp, ALEXA_SCHEMA({}), "mock-user-id", cloud_prefs, cloud_stub
    )
    ...
    assert "alexa" not in opp.config.components
    await cloud_prefs.async_update(
        alexa_default_expose=["sensor"],
    )
    await opp.async_block_till_done()
    assert "alexa" in opp.config.components
    assert not conf.should_expose("light.kitchen")


async def test_alexa_config_report_state(opp, cloud_prefs, cloud_stub):
    """Test Alexa config should expose using prefs."""
    conf = alexa_config.AlexaConfig(
        opp, ALEXA_SCHEMA({}), "mock-user-id", cloud_prefs, cloud_stub
    )
...
async def test_alexa_update_expose_trigger_sync(opp, cloud_prefs, cloud_stub):
...
async def test_alexa_update_report_state(opp, cloud_prefs, cloud_stub):
