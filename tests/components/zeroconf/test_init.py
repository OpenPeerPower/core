...
async def test_setup(opp, mock_zeroconf):
    ...
    with patch.object(
        ...
        "openpeerpower.components.zeroconf.ServiceInfo",
...
async def test_setup_with_overly_long_url_and_name(opp, mock_zeroconf, caplog):
    ...
    with patch.object(opp.config_entries.flow, "async_init"), patch.object(
        ...
        "openpeerpower.components.zeroconf.ServiceInfo.request",
    ):
        assert await async_setup_component(opp, zeroconf.DOMAIN, {zeroconf.DOMAIN: {}})
...
async def test_zeroconf_match_macaddress(opp, mock_zeroconf):
    """Test configured options for a device are loaded via config entry."""
...
async def test_zeroconf_match_manufacturer(opp, mock_zeroconf):
    """Test configured options for a device are loaded via config entry."""
...
async def test_zeroconf_match_manufacturer_not_present(opp, mock_zeroconf):
...
async def test_zeroconf_no_match_manufacturer(opp, mock_zeroconf):
    """Test configured options for a device are loaded via config entry."""
...
async def test_homekit_match_partial_fnmatch(opp, mock_zeroconf):
...
async def test_removed_ignored(opp, mock_zeroconf):
...
async def test_async_detect_interfaces_setting_non_loopback_route(opp):
    """Test without default interface config and the route returns a non-loopback address."""
    with patch(
        "openpeerpower.components.zeroconf.models.HaZeroconf"
    ) as mock_zc, patch.object(opp.config_entries.flow, "async_init"), patch.object(
        zeroconf, "HaServiceBrowser", side_effect=service_update_mock
    ), patch(
        "openpeerpower.components.zeroconf.network.async_get_adapters",
