...
from openpeerpower.components.roomba import config_flow
from openpeerpower.components.roomba.const import CONF_BLID, CONF_CONTINUOUS, DOMAIN
from openpeerpower.const import CONF_DELAY, CONF_HOST, CONF_PASSWORD
...
async def test_form_user_discovery_and_password_fetch(opp):
    ...
    with patch(
        "openpeerpower.components.roomba.config_flow.RoombaFactory.create_roomba",
...
async def test_form_user_no_devices_found_discovery_aborts_already_configured(opp):
...
async def test_form_user_discover_fails_aborts_already_configured(opp):
...
async def test_form_user_discovery_no_devices_found_and_auto_password_fetch(opp):
...
async def test_form_user_discovery_no_devices_found_and_password_fetch_fails(opp):
...
async def test_dhcp_discovery_and_roomba_discovery_finds(opp, discovery_data):
...
async def test_dhcp_discovery_falls_back_to_manual(opp, discovery_data):
...
@pytest.mark.parametrize("discovery_data", DHCP_DISCOVERY_DEVICES_WITHOUT_MATCHING_IP)
async def test_dhcp_discovery_no_devices_falls_back_to_manual(opp, discovery_data):
...
async def test_dhcp_discovery_partial_hostname(opp):
    ...
    current_flows = opp.config_entries.flow.async_progress()
