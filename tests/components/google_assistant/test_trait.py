...
async def test_brightness_light(opp, supported_color_modes):
...
async def test_color_setting_color_light(opp, supported_color_modes):
...
async def test_temperature_control(opp):
...
async def test_preset_modes(opp):
    ...
    calls = async_mock_service(opp, fan.DOMAIN, fan.SERVICE_SET_PRESET_MODE)
    await trt.execute(
        trait.COMMAND_MODES,
        BASIC_DATA,
...
async def test_traits_unknown_domains(opp, caplog):
...
async def test_temperature_control_sensor(opp):
...
async def test_temperature_control_sensor_data(opp, unit_in, unit_out, state, ambient):
    """Test TemperatureControl trait support for temperature sensor."""
...
async def test_channel(opp):
    ...
    trt = trait.ChannelTrait(opp, State("media_player.demo", STATE_ON), BASIC_CONFIG)
    ...
    media_player_calls = async_mock_service(
        opp, media_player.DOMAIN, SERVICE_PLAY_MEDIA
    )
    await trt.execute(
