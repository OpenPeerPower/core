...
from openpeerpower.components.elkm1.const import DOMAIN
...
def mock_elk(invalid_auth=None, sync_complete=None):
    ...
    mocked_elk.add_op.dler.side_effect = op.dler_callbacks
    return mocked_elk
...
async def test_form_user_with_secure_elk.opp):
    ...
    with patch(
        ...
        "openpeerpower.components.elkm1.async_setup", return_value=True
    ) as mock_setup, patch(
