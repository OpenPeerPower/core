...
async def test_number.opp, zha_device_joined_restored, zigpy_analog_output_device):
    ...
    assert.opp.states.get(entity_id).state == "15.0"

    # test attributes
    assert.opp.states.get(entity_id).attributes.get("min") == 0.0
    assert.opp.states.get(entity_id).attributes.get("max") == 100.0
    assert.opp.states.get(entity_id).attributes.get("step") == 1.0
    ...
    assert cluster.read_attributes.call_count == 4
    await send_attributes_report.opp, cluster, {0x0055: 15})
    assert.opp.states.get(entity_id).state == "15.0"

    # update value from device
    await send_attributes_report.opp, cluster, {0x0055: 20})
    assert.opp.states.get(entity_id).state == "20.0"
    ...
    # test rejoin
    assert cluster.read_attributes.call_count == 4
    await async_test_rejoin.opp, zigpy_analog_output_device, [cluster], (1,))
