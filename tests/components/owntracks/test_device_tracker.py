...
# iBeacons that are named the same as an OPP zone
# are used to trigger enter and leave updates
# for that zone. In this case the "inner" zone.
#
# iBeacons that do not share an OPP zone name
...
async def test_event_beacon_unknown_zone_no_location(opp, context):
    """Test the event for unknown zone."""
    # A beacon which does not match a OPP zone is the
...
def config_context(opp, setup_comp):
    ...
    patch_save = patch(
        "openpeerpower.components.device_tracker.DeviceTracker.async_update_config"
    )
    patch_save.start()
...
def mock_not_supports_encryption():
    ...
    with patch(
        "openpeerpower.components.owntracks.messages.supports_encryption",
        return_value=False,
    ):
        yield
...
def mock_get_cipher_error():
    ...
    with patch(
        "openpeerpower.components.owntracks.messages.get_cipher", side_effect=OSError()
    ):
        yield
