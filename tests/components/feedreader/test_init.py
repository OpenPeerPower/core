...
from openpeerpower.components import feedreader
from openpeerpower.components.feedreader import (
    CONF_MAX_ENTRIES,
    CONF_URLS,
    DEFAULT_MAX_ENTRIES,
...
class TestFeedreaderComponent(unittest.TestCase):
    ...
    def test_setup_one_feed(self):
        ...
        with patch(
            "openpeerpower.components.feedreader.track_time_interval"
        ) as track_method:
            assert setup_component(self.opp, feedreader.DOMAIN, VALID_CONFIG_1)
            track_method.assert_called_once_with(
                self.opp, mock.ANY, DEFAULT_SCAN_INTERVAL
            )
    ...
    def test_setup_scan_interval(self):
        ...
        with patch(
            ...
            assert setup_component(self.opp, feedreader.DOMAIN, VALID_CONFIG_2)
            track_method.assert_called_once_with(
                self.opp, mock.ANY, timedelta(seconds=60)
            )

    def test_setup_max_entries(self):
        """Test the setup of this component with max entries."""
        assert setup_component(self.opp, feedreader.DOMAIN, VALID_CONFIG_3)
    ...
    def setup_manager(self, feed_data, max_entries=DEFAULT_MAX_ENTRIES):
        ...
        self.opp.bus.listen(EVENT_FEEDREADER, record_event)
        ...
        with patch(
            ...
            manager = FeedManager(
                feed_data, DEFAULT_SCAN_INTERVAL, max_entries, self.opp, storage
            )
    ...
    def test_feed_updates(self):
        ...
        with patch(
            "openpeerpower.components.feedreader.StoredData.get_timestamp",
    ...
    def test_feed_parsing_failed(self, mock_parse):
        ...
        storage = StoredData(data_file)
        manager = FeedManager(
            "FEED DATA", DEFAULT_SCAN_INTERVAL, DEFAULT_MAX_ENTRIES, self.opp, storage
        )
        # Artificially trigger update.
        self.opp.bus.fire(EVENT_OPENPEERPOWER_START)
        # Collect events.
