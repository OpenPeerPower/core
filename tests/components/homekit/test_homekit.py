...
async def test_homekit_add_accessory(opp, mock_zeroconf):
    ...
    homekit = _mock_homekit_bridge(opp, entry)
    ...
    with patch(f"{PATH_HOMEKIT}.get_accessory") as mock_get_acc:
        ...
        mock_get_acc.assert_called_with(opp, ANY, ANY, 1403373688, {})
        assert not homekit.bridge.add_accessory.called

        state = State("demo.test", "on")
        homekit.add_bridge_accessory(state)
        mock_get_acc.assert_called_with(opp, ANY, ANY, 600325356, {})
        assert homekit.bridge.add_accessory.called

        state = State("demo.test_2", "on")
        homekit.add_bridge_accessory(state)
        mock_get_acc.assert_called_with(opp, ANY, ANY, 1467253281, {})
        assert homekit.bridge.add_accessory.called
...
async def test_homekit_warn_add_accessory_bridge(
    ...
    with patch(f"{PATH_HOMEKIT}.get_accessory") as mock_get_acc:
        ...
        homekit.add_bridge_accessory(state)
        mock_get_acc.assert_called_with(opp, ANY, ANY, 1508819236, {})
        assert not homekit.bridge.add_accessory.called
