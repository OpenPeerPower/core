...
import openpeerpower.components.facebox.image_processing as fb
...
def mock_healthybox():
    ...
    check_box_health = (
        "openpeerpower.components.facebox.image_processing.check_box_health"
    )
...
def mock_isfile():
    """Mock os.path.isfile."""
    with patch(
        "openpeerpower.components.facebox.image_processing.cv.isfile", return_value=True
    ) as _mock_isfile:
        yield _mock_isfile
...
def mock_image():
    ...
    with patch(
        "openpeerpower.components.demo.camera.DemoCamera.camera_image",
        return_value=b"Test",
    ) as image:
        yield image
...
def mock_open_file():
    ...
    mopen = mock_open()
    with patch(
        "openpeerpower.components.facebox.image_processing.open", mopen, create=True
    ) as _mock_open:
        yield _mock_open
