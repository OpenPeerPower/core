...
class TestLiteJetSwitch(unittest.TestCase):
    ...
    def setup_method(self, method, mock_pylitejet):
        ...
        assert setup.setup_component(self.opp, litejet.DOMAIN, config)
    ...
    def switch(self):
        ...
        return self.opp.states.get(ENTITY_SWITCH)

    def other_switch(self):
        """Return the other switch state."""
        return self.opp.states.get(ENTITY_OTHER_SWITCH)
    ...
    def test_on_off(self):
        ...
        assert not switch.is_on(self.opp, ENTITY_SWITCH)

        common.turn_on(self.opp, ENTITY_SWITCH)
        self.opp.block_till_done()
        self.mock_lj.press_switch.assert_called_with(ENTITY_SWITCH_NUMBER)

        common.turn_off(self.opp, ENTITY_SWITCH)
        self.opp.block_till_done()
    ...
    def test_pressed_event(self):
        ...
        assert switch.is_on(self.opp, ENTITY_SWITCH)
        assert not switch.is_on(self.opp, ENTITY_OTHER_SWITCH)
        ...
        assert switch.is_on(self.opp, ENTITY_OTHER_SWITCH)
        assert switch.is_on(self.opp, ENTITY_SWITCH)
