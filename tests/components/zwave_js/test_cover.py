...
async def test_blind_cover(opp, client, iblinds_v2, integration):
    """Test a blind cover entity."""
    state = opp.states.get(BLIND_COVER_ENTITY)

    assert state
    assert state.attributes[ATTR_DEVICE_CLASS] == DEVICE_CLASS_BLIND


async def test_shutter_cover(opp, client, qubino_shutter, integration):
    """Test a shutter cover entity."""
    state = opp.states.get(SHUTTER_COVER_ENTITY)
