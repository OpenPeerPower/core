...
async def test_thermostat_heatit_z_trm3_no_value(
    opp, client, climate_heatit_z_trm3_no_value, integration
):
...
async def test_thermostat_heatit_z_trm3(
    opp, client, climate_heatit_z_trm3, integration
):
...
async def test_thermostat_heatit_z_trm2fx(
    opp, client, climate_heatit_z_trm2fx, integration
):
...
async def test_thermostat_srt321_hrt4_zw(opp, client, srt321_hrt4_zw, integration):
    """Test a climate entity from a HRT4-ZW / SRT321 thermostat device.

    This device currently has no setpoint values.
    """
    state = opp.states.get(CLIMATE_MAIN_HEAT_ACTIONNER)
...
async def test_preset_and_no_setpoint(
    opp, client, climate_eurotronic_spirit_z, integration
):
    """Test preset without setpoint value."""
    node = climate_eurotronic_spirit_z

    state = opp.states.get(CLIMATE_EUROTRONICS_SPIRIT_Z_ENTITY)
    assert state
    assert state.state == HVAC_MODE_HEAT
...
async def test_temp_unit_fix(
    ...
    state = opp.states.get("climate.z_wave_thermostat")
    assert state
