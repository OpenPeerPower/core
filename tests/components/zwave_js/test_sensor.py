...
async def test_node_status_sensor(opp, lock_id_lock_as_id150, integration):
    ...
    assert opp.states.get(NODE_STATUS_ENTITY).state == "alive"
    ...
    assert opp.states.get(NODE_STATUS_ENTITY).state == "dead"

    event = Event(
        "wake up", data={"source": "node", "event": "wake up", "nodeId": node.node_id}
    )
    node.receive_event(event)
    assert opp.states.get(NODE_STATUS_ENTITY).state == "awake"

    event = Event(
        "sleep", data={"source": "node", "event": "sleep", "nodeId": node.node_id}
    )
    node.receive_event(event)
    assert opp.states.get(NODE_STATUS_ENTITY).state == "asleep"
