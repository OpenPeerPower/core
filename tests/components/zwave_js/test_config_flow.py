...
from openpeerpower.components.zwave_js.config_flow import TITLE
...
def mock_start_addon(start_addon_side_effect):
    """Mock start add-on."""
    with patch(
        "openpeerpower.components.oppio.async_start_addon",
...
def mock_addon_setup_time():
    ...
    with patch(
        "openpeerpower.components.zwave_js.config_flow.ADDON_SETUP_TIME", new=0
    ) as addon_setup_time:
