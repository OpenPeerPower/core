...
def test_multilevelsensor_value_changed_other_units(mock_openzwave):
    ...
    value = MockValue(
        data=190.95555, units=openpeerpower.const.ENERGY_KILO_WATT_HOUR, node=node
    )
    values = MockEntityValues(primary=value)

    device = sensor.get_device(node=node, values=values, node_config={})
    assert device.state == 190.96
    assert device.unit_of_measurement == openpeerpower.const.ENERGY_KILO_WATT_HOUR
    value.data = 197.95555
    value_changed(value)
...
def test_alarm_sensor_value_changed(mock_openzwave):
    ...
    value = MockValue(data=12.34, node=node, units=openpeerpower.const.PERCENTAGE)
    values = MockEntityValues(primary=value)

    device = sensor.get_device(node=node, values=values, node_config={})
    assert device.state == 12.34
    assert device.unit_of_measurement == openpeerpower.const.PERCENTAGE
    value.data = 45.67
    value_changed(value)
