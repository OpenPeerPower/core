...
from openpeerpower.components.harmony.const import ACTIVITY_POWER_OFF
...
def mock_hc():
    ...
    with patch(
        "openpeerpower.components.harmony.data.HarmonyClient",
...
def mock_write_config():
    ...
    with patch(
        "openpeerpower.components.harmony.remote.HarmonyRemote.write_config_file",
    ) as mock:
        yield mock
