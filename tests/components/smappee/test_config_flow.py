...
async def test_show_zeroconf_connection_error_form_next_generation(opp):
    """Test that the zeroconf confirmation form is served."""
...
async def test_user_local_connection_error(opp):
...
async def test_full_zeroconf_flow_next_generation(opp):
    """Test the full zeroconf flow."""
    with patch(
        "pysmappee.mqtt.SmappeeLocalMqtt.start_attempt", return_value=True
