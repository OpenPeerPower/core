...
def gen_platform_strings_schema(config: Config, integration: Integration):
    ...
    def device_class_validator(value):
        ...
        if not value.startswith(f"{integration.domain}__"):
            raise vol.Invalid(
                f"Device class need to start with '{integration.domain}__'. Key {value} is invalid. See https://developers.open-peer-power.io/docs/internationalization/core#stringssensorjson"
            )
