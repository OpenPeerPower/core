...
class OppConstructorFormatChecker(BaseChecker):  # type: ignore[misc]
    """Checker for __init__ definitions."""

    __implements__ = IAstroidChecker

    name = "opp_constructor"
    priority = -1
    msgs = {
        "W0006": (
            '__init__ should have explicit return type "None"',
            "opp-constructor-return",
    ...
    def visit_functiondef(self, node: FunctionDef) -> None:
        ...
        if not isinstance(node.returns, Const) or node.returns.value is not None:
            self.add_message("opp-constructor-return", node=node)


def register(linter: PyLinter) -> None:
    """Register the checker."""
    linter.register_checker(OppConstructorFormatChecker(linter))
