...
class OppLoggerFormatChecker(BaseChecker):  # type: ignore[misc]
    ...
    def visit_call(self, node: astroid.Call) -> None:
        ...
        if log_message[-1] == ".":
            self.add_message("opp-logger-period", node=node)
        ...
        if (
            ...
            self.add_message("opp-logger-capital", node=node)


def register(linter: PyLinter) -> None:
    """Register the checker."""
    linter.register_checker(OppLoggerFormatChecker(linter))
