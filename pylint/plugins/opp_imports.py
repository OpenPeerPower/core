...
class OppImportsFormatChecker(BaseChecker):  # type: ignore[misc]
    """Checker for imports."""
    ...
    def visit_import(self, node: Import) -> None:
        ...
        for module, _alias in node.names:
            if module.startswith(f"{self.current_module}."):
                self.add_message("opp-relative-import", node=node)
...
def register(linter: PyLinter) -> None:
    ...
    linter.register_checker(OppImportsFormatChecker(linter))
