...
from openpeerpower.loader import Integration, IntegrationNotFound, async_get_integration
...
async def async_get_integration_with_requirements(
    opp: OpenPeerPower, domain: str, done: set[str] | None = None
) -> Integration:
    ...
    try:
        await _async_process_integration(opp, integration, done)
    except Exception:
        del cache[domain]
        event.set()
        raise
...
async def _async_process_integration(
    opp: OpenPeerPower, integration: Integration, done: set[str]
) -> None:
