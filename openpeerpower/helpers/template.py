...
class Template:
    ...
    def _env(self) -> TemplateEnvironment:
        ...
        ret: TemplateEnvironment | None = self.opp.data.get(wanted_env)
        if ret is None:
            ret = self.opp.data[wanted_env] = TemplateEnvironment(self.opp, self._limited, self._strict)  # type: ignore[no-untyped-call]
        return ret
    ...
    def async_render_to_info(
        ...
        self.opp.data[_RENDER_INFO] = render_info
        try:
...
class AllStates:
    """Class to expose all OPP states as attributes."""
...
class DomainStates:
    """Class to expose a specific OPP domain as attributes."""

    def __init__(self, opp: OpenPeerPower, domain: str) -> None:
...
class TemplateState(State):
    ...
    # pylint: disable=super-init-not-called
    def __init__(self, opp: OpenPeerPower, state: State, collect: bool = True) -> None:
...
def _collect_state(opp: OpenPeerPower, entity_id: str) -> None:
    entity_collect = opp.data.get(_RENDER_INFO)
    if entity_collect is not None:
        entity_collect.entities.add(entity_id)


def _state_generator(opp: OpenPeerPower, domain: str | None) -> Generator:
...
def _get_state_if_valid(opp: OpenPeerPower, entity_id: str) -> TemplateState | None:
...
def _get_state(opp: OpenPeerPower, entity_id: str) -> TemplateState | None:
    return _get_template_state_from_state(opp, entity_id, opp.states.get(entity_id))


def _get_template_state_from_state(
    opp: OpenPeerPower, entity_id: str, state: State | None
) -> TemplateState | None:
    if state is None:
...
def _resolve_state(
    opp: OpenPeerPower, entity_id_or_state: Any
...
def result_as_boolean(template_result: str | None) -> bool:
    ...
    try:
        ...
        from openpeerpower.helpers import (  # pylint: disable=import-outside-toplevel
...
def expand(opp: OpenPeerPower, *args: Any) -> Iterable[State]:
...
def device_entities(opp: OpenPeerPower, device_id: str) -> Iterable[str]:
...
def is_state(opp: OpenPeerPower, entity_id: str, state: State) -> bool:
    """Test if a state is a specific value."""
    state_obj = _get_state(opp, entity_id)
    return state_obj is not None and state_obj.state == state


def is_state_attr(opp: OpenPeerPower, entity_id: str, name: str, value: Any) -> bool:
    """Test if a state's attribute is a specific value."""
    attr = state_attr(opp, entity_id, name)
    return attr is not None and attr == value


def state_attr(opp: OpenPeerPower, entity_id: str, name: str) -> Any:
...
def utcnow(opp: OpenPeerPower) -> datetime:
...
class TemplateEnvironment(ImmutableSandboxedEnvironment):
    ...
    def __init__(self, opp, limited=False, strict=False):
