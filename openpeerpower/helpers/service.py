...
from openpeerpower.core import Context, OpenPeerPower, ServiceCall, callback
...
def extract_entity_ids(
    opp: OpenPeerPower, service_call: ServiceCall, expand_group: bool = True
) -> set[str]:
...
async def async_extract_referenced_entity_ids(
    ...
    area_reg = area_registry.async_get(opp)
...
def _load_services_files(
    opp: OpenPeerPower, integrations: Iterable[Integration]
) -> list[JSON_TYPE]:
...
def async_set_service_schema(
    opp: OpenPeerPower, domain: str, service: str, schema: dict[str, Any]
) -> None:
