            ...
            .with_query(
                {
                    ...
                    "state": _encode_jwt(
                        self.opp, {"flow_id": flow_id, "redirect_uri": redirect_uri}
                    ),
                }
            )
...
def async_register_implementation(
    ...
    if isinstance(implementation, LocalOAuth2Implementation) and not opp.data.get(
        ...
        opp.http.register_view(OAuth2AuthorizeCallbackView())
...
def async_add_implementation_provider(
    ...
    async_provide_implementation: Callable[
        [OpenPeerPower, str], Awaitable[AbstractOAuth2Implementation | None]
    ],
) -> None:
...
class OAuth2AuthorizeCallbackView(http.OpenPeerPowerView):
...
@callback
def _decode_jwt(opp: OpenPeerPower, encoded: str) -> dict | None:
    """JWT encode data."""
