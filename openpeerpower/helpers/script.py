...
from openpeerpower.helpers.condition import (
...
from openpeerpower.helpers.trace import script_execution_set
...
async def trace_action(opp, script_run, stop, variables):
    ...
    if trace_id:
        ...
        if key in breakpoints and (
            ...
            async_dispatcher_send(opp, SCRIPT_BREAKPOINT_HIT, key, run_id, path)
            ...
            remove_signal1 = async_dispatcher_connect(opp, signal, async_continue_stop)
            remove_signal2 = async_dispatcher_connect(
                opp, SCRIPT_DEBUG_CONTINUE_ALL, async_continue_stop
            )

            tasks = [opp.async_create_task(flag.wait()) for flag in (stop, done)]
...
class _ScriptRun:
    ...
    async def _async_step(self, log_exceptions):
        with trace_path(str(self._step)):
            async with trace_action(self._opp, self, self._stop, self._variables):
                if self._stop.is_set():
                    return
                try:
    ...
    async def _async_wait_template_step(self):
        ...
        if condition.async_template(self._opp, wait_template, self._variables, False):
    ...
    def _test_conditions(self, conditions, name, condition_path=None):
        ...
        @trace_condition_function
        def traced_test_conditions(opp, variables):
            try:
                with trace_path(condition_path):
                    for idx, cond in enumerate(conditions):
                        with trace_path(str(idx)):
                            if not cond(opp, variables):
                                return False
            except exceptions.ConditionError as ex:
        ...
        result = traced_test_conditions(self._opp, self._variables)
        return result
...
@callback
def breakpoint_clear(opp, key, run_id, node):
    """Clear a breakpoint."""
...
@callback
def breakpoint_set(opp, key, run_id, node):
    """Set a breakpoint."""
    run_id = run_id or RUN_ID_ANY
    breakpoints = opp.data[DATA_SCRIPT_BREAKPOINTS]
...
@callback
def breakpoint_list(opp: OpenPeerPower) -> list[dict[str, Any]]:
    """List breakpoints."""
    breakpoints = opp.data[DATA_SCRIPT_BREAKPOINTS]
...
@callback
def debug_continue(opp, key, run_id):
    """Continue execution of a halted script."""
    # Clear any wildcard breakpoint
    breakpoint_clear(opp, key, run_id, NODE_ANY)

    signal = SCRIPT_DEBUG_CONTINUE_STOP.format(key, run_id)
    async_dispatcher_send(opp, signal, "continue")


@callback
def debug_step(opp, key, run_id):
    """Single step a halted script."""
    # Set a wildcard breakpoint
    breakpoint_set(opp, key, run_id, NODE_ANY)
...
@callback
def debug_stop(opp, key, run_id):
    """Stop execution of a running or halted script."""
    signal = SCRIPT_DEBUG_CONTINUE_STOP.format(key, run_id)
    async_dispatcher_send(opp, signal, "stop")
