...
from openpeerpower.generated.dhcp import DHCP
from openpeerpower.generated.mqtt import MQTT
from openpeerpower.generated.ssdp import SSDP
from openpeerpower.generated.zeroconf import HOMEKIT, ZEROCONF
...
async def async_get_zeroconf(opp: OpenPeerPower) -> dict[str, list[dict[str, str]]]:
...
async def async_get_dhcp(opp: OpenPeerPower) -> list[dict[str, str]]:
...
async def async_get_homekit(opp: OpenPeerPower) -> dict[str, str]:
...
async def async_get_ssdp(opp: OpenPeerPower) -> dict[str, list[dict[str, str]]]:
...
async def async_get_mqtt(opp: OpenPeerPower) -> dict[str, list[str]]:
        ...
        for base in root_module.__path__:  # type: ignore
            ...
            try:
                ...
                _LOGGER.error(
                    ...
                    "See https://developers.openpeerpower.io/blog/2021/01/29/custom-integration-changes#versions for more details",
                    integration.domain,
                    integration.version,
                )
                return None
...
async def async_get_integration(opp: OpenPeerPower, domain: str) -> Integration:
    ...
    try:
        integration = await _async_get_integration(opp, domain)
...
async def _async_get_integration(opp: OpenPeerPower, domain: str) -> Integration:
    # Instead of using resolve_from_root we use the cache of custom
    # components to find the integration.
    if integration := (await async_get_custom_components(opp)).get(domain):
        return integration

    from openpeerpower import components  # pylint: disable=import-outside-toplevel

    if integration := await opp.async_add_executor_job(
...
async def _async_component_dependencies(
    ...
    for dependency_domain in integration.dependencies:
        ...
        dep_integration = await async_get_integration(opp, dependency_domain)
