...
from openpeerpower.helpers.typing import UNDEFINED, DiscoveryInfoType, UndefinedType
...
UpdateListenerType = Callable[[OpenPeerPower, "ConfigEntry"], Any]
...
class ConfigEntry:
    ...
    async def async_unload(
        self, opp: OpenPeerPower, *, integration: loader.Integration | None = None
    ) -> bool:
        """Unload an entry.
    ...
    @callback
    def async_start_reauth(self, opp: OpenPeerPower) -> None:
        """Start a reauth flow."""
        flow_context = {
...
class ConfigEntries:
    ...
    def async_setup_platforms(
        ...
        for platform in platforms:
            self.opp.async_create_task(self.async_forward_entry_setup(entry, platform))
...
class ConfigFlow(data_entry_flow.FlowHandler):
        ...
        for entry in self._async_current_entries(include_ignore=True):
            if entry.unique_id == self.unique_id:
                if updates is not None:
                    changed = self.opp.config_entries.async_update_entry(
                    ...
                    if (
                        ...
                        self.opp.async_create_task(
                            self.opp.config_entries.async_reload(entry.entry_id)
                        )
        ...
        if unique_id != DEFAULT_DISCOVERY_UNIQUE_ID:
            for progress in self._async_in_progress():
                if progress["context"].get("unique_id") == DEFAULT_DISCOVERY_UNIQUE_ID:
                    self.opp.config_entries.flow.async_abort(progress["flow_id"])
    ...
    def _async_current_entries(
        ...
        """
        config_entries = self.opp.config_entries.async_entries(self.handler)
    ...
    async def async_step_oppio(
        self, discovery_info: DiscoveryInfoType
    ) -> data_entry_flow.FlowResult:
        """Handle a flow initialized by OPP IO discovery."""
