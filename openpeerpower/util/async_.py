...
def run_callback_threadsafe(
    ...
    if hasattr(loop, _SHUTDOWN_RUN_CALLBACK_THREADSAFE):
        #
        # If the final `OpenPeerPower.async_block_till_done` in
        # `OpenPeerPower.async_stop` has already been called, the callback
...
def check_loop() -> None:
    ...
    for frame in reversed(extract_stack()):
        for path in ("custom_components/", "openpeerpower/components/"):
            try:
    ...
    raise RuntimeError(
        f"I/O must be done in the executor; Use `await opp.async_add_executor_job()` "
...
def shutdown_run_callback_threadsafe(loop: AbstractEventLoop) -> None:
    ...
    be called when Open Peer Power is going to shutdown and
