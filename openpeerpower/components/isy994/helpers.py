...
from openpeerpower.components.climate.const import DOMAIN as CLIMATE
...
def _check_for_node_def(
    opp_isy_data: dict, node: Group | Node, single_platform: str = None
) -> bool:
...
def _is_sensor_a_binary_sensor(opp_isy_data: dict, node: Group | Node) -> bool:
...
async def migrate_old_unique_ids(
    opp: OpenPeerPower, platform: str, devices: list[Any] | None
) -> None:
