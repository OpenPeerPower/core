...
class SmartThingsFlowHandler(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input=None):
        ...
        await setup_smartapp_endpoint(self.opp)
        webhook_url = get_webhook_url(self.opp)

        # Abort if the webhook is invalid
        if not validate_webhook_requirements(self.opp):
    ...
    async def async_step_pat(self, user_input=None):
        ...
        # Setup end-point
        self.api = SmartThings(async_get_clientsession(self.opp), self.access_token)
        try:
            app = await find_app(self.opp, self.api)
            if app:
                await app.refresh()  # load all attributes
                await update_app(self.opp, app)
                ...
                app, client = await create_app(self.opp, self.api)
                self.oauth_client_secret = client.client_secret
                self.oauth_client_id = client.client_id
            setup_smartapp(self.opp, app)
