...
class BraviaTVConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def init_device(self, pin):
        ...
        connected = await self.opp.async_add_executor_job(self.braviarc.is_connected)
        if not connected:
            raise CannotConnect()

        system_info = await self.opp.async_add_executor_job(
...
class BraviaTVOptionsFlowHandler(config_entries.OptionsFlow):
    ...
    async def async_step_init(self, user_input=None):
        """Manage the options."""
        self.braviarc = self.opp.data[DOMAIN][self.config_entry.entry_id][BRAVIARC]
        connected = await self.opp.async_add_executor_job(self.braviarc.is_connected)
        if not connected:
            await self.opp.async_add_executor_job(
        ...
        content_mapping = await self.opp.async_add_executor_job(
...
class ModelNotSupported(exceptions.OpenPeerPowerError):
