...
from openpeerpower.const import CONF_HOST, CONF_NAME, CONF_PIN, STATE_OFF, STATE_ON
...
async def async_setup_platform(opp, config, async_add_entities, discovery_info=None):
    ...
    bravia_config = await opp.async_add_executor_job(load_json, bravia_config_file_path)
...
class BraviaTVDevice(MediaPlayerEntity):
    ...
    async def async_update(self):
        ...
        power_status = await self.opp.async_add_executor_job(
            self._braviarc.get_power_status
        )

        if power_status != "off":
            connected = await self.opp.async_add_executor_job(
    ...
    async def _async_refresh_volume(self):
        ...
        volume_info = await self.opp.async_add_executor_job(
    ...
    async def _async_refresh_channels(self):
        ...
        if not self._source_list:
            self._content_mapping = await self.opp.async_add_executor_job(
    ...
    async def _async_refresh_playing_info(self):
        ...
        playing_info = await self.opp.async_add_executor_job(
    ...
    async def async_turn_on(self):
        ...
        async with self._state_lock:
            await self.opp.async_add_executor_job(self._braviarc.turn_on)

    async def async_turn_off(self):
        """Turn off media player."""
        async with self._state_lock:
            await self.opp.async_add_executor_job(self._braviarc.turn_off)
