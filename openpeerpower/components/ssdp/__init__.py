...
from openpeerpower.core import CoreState, OpenPeerPower, callback as core_callback
from openpeerpower.helpers.event import async_track_time_interval
from openpeerpower.helpers.typing import ConfigType
from openpeerpower.loader import async_get_ssdp, bind_opp
...
def async_register_callback(
    ...
    """
    scanner: Scanner = opp.data[DOMAIN]
...
def async_get_discovery_info_by_udn_st(  # pylint: disable=invalid-name
    opp: OpenPeerPower, udn: str, st: str
) -> dict[str, str] | None:
...
async def async_setup(opp: OpenPeerPower, config: ConfigType) -> bool:
    ...
    scanner = opp.data[DOMAIN] = Scanner(opp, await async_get_ssdp(opp))
...
class Scanner:
    ...
    def __init__(
        self, opp: OpenPeerPower, integration_matchers: dict[str, list[dict[str, str]]]
    ) -> None:
        """Initialize class."""
        self.opp = opp
    ...
    async def _async_build_source_set(self) -> set[IPv4Address | IPv6Address]:
        ...
        adapters = await network.async_get_adapters(self.opp)
    ...
    async def async_start(self) -> None:
        """Start the scanner."""
        self.description_manager = DescriptionManager(self.opp)
        ...
        self.opp.bus.async_listen_once(
            EVENT_OPENPEERPOWER_STARTED, self.flow_dispatcher.async_start
        )
        await asyncio.gather(
            *[listener.async_start() for listener in self._ssdp_listeners]
        )
        self._cancel_scan = async_track_time_interval(
            self.opp, self.async_scan, SCAN_INTERVAL
        )
