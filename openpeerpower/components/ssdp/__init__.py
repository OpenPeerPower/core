...
async def async_setup(opp, config):
    ...
    async def _async_initialize(_):
        ...
        cancel_scan = async_track_time_interval(opp, scanner.async_scan, SCAN_INTERVAL)

        @callback
        def _async_stop_scans(event):
            cancel_scan()

        opp.bus.async_listen_once(EVENT_OPENPEERPOWER_STOP, _async_stop_scans)

    opp.bus.async_listen_once(EVENT_OPENPEERPOWER_STARTED, _async_initialize)
