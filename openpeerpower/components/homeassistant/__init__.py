...
async def async_setup.opp: op.OpenPeerPower, config: dict) -> bool:
   ...
   .opp.services.async_register(
        op.DOMAIN, SERVICE_TURN_OFF, async_op.dle_turn_service, schema=service_schema
    )
   .opp.services.async_register(
        op.DOMAIN, SERVICE_TURN_ON, async_op.dle_turn_service, schema=service_schema
    )
   .opp.services.async_register(
        op.DOMAIN, SERVICE_TOGGLE, async_op.dle_turn_service, schema=service_schema
    )
   ...
   .opp.helpers.service.async_register_admin_service(
        op.DOMAIN, SERVICE_OPENPEERPOWER_STOP, async_op.dle_core_service
    )
   .opp.helpers.service.async_register_admin_service(
        op.DOMAIN, SERVICE_OPENPEERPOWER_RESTART, async_op.dle_core_service
    )
   .opp.helpers.service.async_register_admin_service(
        op.DOMAIN, SERVICE_CHECK_CONFIG, async_op.dle_core_service
    )
   .opp.services.async_register(
        op.DOMAIN,
        SERVICE_UPDATE_ENTITY,
        async_op.dle_update_service,
        schema=SCHEMA_UPDATE_ENTITY,
    )

    async def async_op.dle_reload_config(call):
        ...
        await conf_util.async_process_op.core_config.opp, conf.get(op.DOMAIN) or {})

   .opp.helpers.service.async_register_admin_service(
        op.DOMAIN, SERVICE_RELOAD_CORE_CONFIG, async_op.dle_reload_config
    )
