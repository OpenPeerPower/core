...
from openpeerpower.helpers.device_registry import async_get as get_dev_reg
from openpeerpower.helpers.entity import Entity

from .const import DOMAIN


async def async_remove_devices(opp, entity, entry_id):
    """Get item that is removed from session."""
    dev_registry = get_dev_reg(opp)
    ...
    async def async_added_to_opp(self):
        ...
        def update_entity_information():
            if self._device.deleted:
                self.opp.add_job(async_remove_devices(self.opp, self, self._entry_id))
            else:
                self.schedule_update_op_state()
