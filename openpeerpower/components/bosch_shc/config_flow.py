...
from openpeerpower.const import CONF_HOST, CONF_PASSWORD, CONF_TOKEN
...
def write_tls_asset(opp: core.OpenPeerPower, filename: str, asset: bytes) -> None:
    """Write the tls assets to disk."""
    makedirs(opp.config.path(DOMAIN), exist_ok=True)
    with open(opp.config.path(DOMAIN, filename), "w") as file_handle:
        file_handle.write(asset.decode("utf-8"))


def create_credentials_and_validate(opp, host, user_input, zeroconf):
    """Create and store credentials and validate session."""
    helper = SHCRegisterClient(host, user_input[CONF_PASSWORD])
    result = helper.register(host, "OpenPeerPower")

    if result is not None:
        write_tls_asset(opp, CONF_SHC_CERT, result["cert"])
        write_tls_asset(opp, CONF_SHC_KEY, result["key"])

        session = SHCSession(
            host,
            opp.config.path(DOMAIN, CONF_SHC_CERT),
            opp.config.path(DOMAIN, CONF_SHC_KEY),
            True,
            zeroconf,
        )
...
def get_info_from_host(opp, host, zeroconf):
...
class ConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_credentials(self, user_input=None):
        ...
        errors = {}
        if user_input is not None:
            zeroconf = await async_get_instance(self.opp)
            try:
                ...
                entry_data = {
                    CONF_SSL_CERTIFICATE: self.opp.config.path(DOMAIN, CONF_SHC_CERT),
                    CONF_SSL_KEY: self.opp.config.path(DOMAIN, CONF_SHC_KEY),
                    CONF_HOST: self.host,
