...
from openpeerpower.components.ssdp import ATTR_UPNP_MANUFACTURER, ATTR_UPNP_MODEL_NAME
...
class KonnectedFlowHandler(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_gen_config(self, host, port):
        ...
        try:
            status = await get_status(self.opp, host, port)
        ...
        if user_input:
            ...
            try:
                status = await get_status(
                    self.opp, self.data[CONF_HOST], self.data[CONF_PORT]
                )
    ...
    async def async_step_confirm(self, user_input=None):
        ...
        self.data[CONF_ACCESS_TOKEN] = self.opp.data.get(DOMAIN, {}).get(
            CONF_ACCESS_TOKEN
