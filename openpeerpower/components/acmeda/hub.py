...
class PulseHub:
    ...
    async def async_notify_update(self, update_type):
        ...
        if update_type == aiopulse.UpdateType.rollers:
            await update_devices(self.opp, self.config_entry, self.api.rollers)
            self.opp.config_entries.async_update_entry(
                self.config_entry, title=self.title
            )

            async_dispatcher_send(
                self.opp, ACMEDA_HUB_UPDATE.format(self.config_entry.entry_id)
            )
            ...
            for unique_id in list(self.current_rollers):
                if unique_id not in self.api.rollers:
                    ...
                    async_dispatcher_send(
                        self.opp, ACMEDA_ENTITY_REMOVE.format(unique_id)
                    )
