...
class InsteonEntity(Entity):
    ...
    async def async_added_to_opp(self):
        ...
        self.async_on_remove(
            async_dispatcher_connect(self.opp, load_signal, self._async_read_aldb)
        )
        print_signal = f"{self.entity_id}_{SIGNAL_PRINT_ALDB}"
        async_dispatcher_connect(self.opp, print_signal, self._print_aldb)
        default_links_signal = f"{self.entity_id}_{SIGNAL_ADD_DEFAULT_LINKS}"
        async_dispatcher_connect(
            self.opp, default_links_signal, self._async_add_default_links
        )
    ...
    async def _async_read_aldb(self, reload):
        ...
        self._print_aldb()
        async_dispatcher_send(self.opp, SIGNAL_SAVE_DEVICES)
