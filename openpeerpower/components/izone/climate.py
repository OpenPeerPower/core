...
async def async_setup_entry(opp: OpenPeerPower, config: ConfigType, async_add_entities):
    ...
    def init_controller(ctrl: Controller):
        ...
        conf: ConfigType = opp.data.get(DATA_CONFIG)
    ...
    async def async_added_to_opp(self):
        ...
        self.async_on_remove(
            async_dispatcher_connect(
                self.opp, DISPATCH_CONTROLLER_DISCONNECTED, controller_disconnected
            )
        )
        ...
        self.async_on_remove(
            async_dispatcher_connect(
                self.opp, DISPATCH_CONTROLLER_RECONNECTED, controller_reconnected
            )
        )
        ...
        def controller_update(ctrl: Controller) -> None:
            ...
            for zone in self.zones.values():
                zone.async_schedule_update_op_state()

        self.async_on_remove(
            async_dispatcher_connect(
                self.opp, DISPATCH_CONTROLLER_UPDATE, controller_update
            )
        )
