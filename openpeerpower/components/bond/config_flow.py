...
async def _validate_input(opp: OpenPeerPower, data: dict[str, Any]) -> tuple[str, str]:
    """Validate the user input allows us to connect."""

    bond = Bond(
        data[CONF_HOST], data[CONF_ACCESS_TOKEN], session=async_get_clientsession(opp)
    )
    try:
        hub = BondHub(bond)
...
class ConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def _async_try_automatic_configure(self) -> None:
        ...
        """
        bond = Bond(
            self._discovered[CONF_HOST], "", session=async_get_clientsession(self.opp)
        )
        try:
        ...
        _, hub_name = await _validate_input(self.opp, self._discovered)
    ...
    async def async_step_confirm(
        ...
        if user_input is not None:
            ...
            try:
                _, hub_name = await _validate_input(self.opp, data)
    ...
    async def async_step_user(
        ...
        if user_input is not None:
            try:
                bond_id, hub_name = await _validate_input(self.opp, user_input)
            except InputValidationError as error:
                errors["base"] = error.base
            else:
                await self.async_set_unique_id(bond_id)
