...
import openpeerpower.helpers.template as template_helper
   ...
   .opp,
    ...
    class SendNotificationBot(slixmpp.ClientXMPP):
        ...
        def __init__(self):
            ...
            self.use_ipv6 = False
            self.add_event_op.dler("failed_auth", self.disconnect_on_login_fail)
            self.add_event_op.dler("session_start", self.start)

            if room:
                self.register_plugin("xep_0045")  # MUC
            if not verify_certificate:
                self.add_event_op.dler(
