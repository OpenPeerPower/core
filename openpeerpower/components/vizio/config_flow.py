...
from openpeerpower.util.network import is_ip_address
            ...
            options = options.extend(
                {
                    ...
                    vol.Optional(
                        ...
                        [
                            ...
                            *[
                                app["name"]
                                for app in self.opp.data[DOMAIN][CONF_APPS].data
                            ],
                        ]
                    ),
                }
...
class VizioConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
        ...
        if user_input is not None:
            ...
            if not errors:
                if self._must_show_form and self.context["source"] == SOURCE_ZEROCONF:
                    ...
                    if not await VizioAsync.validate_op_config(
                        user_input[CONF_HOST],
    ...
    async def async_step_import(self, import_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for entry in self.opp.config_entries.async_entries(DOMAIN):
            ...
            if await self.opp.async_add_executor_job(
