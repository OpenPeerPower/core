...
class IcloudFlowHandler(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def _validate_and_create_entry(self, user_input, step_id):
        ...
        try:
            self.api = await self.opp.async_add_executor_job(
                PyiCloudService,
                self._username,
                self._password,
                self.opp.helpers.storage.Store(STORAGE_VERSION, STORAGE_KEY).path,
                True,
                None,
                self._with_family,
            )
        ...
        try:
            devices = await self.opp.async_add_executor_job(
        ...
        for entry in self.opp.config_entries.async_entries(DOMAIN):
            if entry.unique_id == self.unique_id:
                self.opp.config_entries.async_update_entry(entry, data=data)
    ...
    async def async_step_user(self, user_input=None):
        ...
        icloud_dir = self.opp.helpers.storage.Store(STORAGE_VERSION, STORAGE_KEY)

        if not os.path.exists(icloud_dir.path):
            await self.opp.async_add_executor_job(os.makedirs, icloud_dir.path)
    ...
    async def async_step_trusted_device(self, user_input=None, errors=None):
        ...
        trusted_devices = await self.opp.async_add_executor_job(
        ...
        if not await self.opp.async_add_executor_job(
            ...
            if self.api.requires_2fa:
                try:
                    self.api = await self.opp.async_add_executor_job(
                        ...
                        self._username,
                        self._password,
                        self.opp.helpers.storage.Store(
