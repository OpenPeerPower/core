...
from openpeerpower.config_entries import CONN_CLASS_ASSUMED
...
class EnOceanFlowHandler(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_detect(self, user_input=None):
        ...
        bridges = await self.opp.async_add_executor_job(dongle.detect)
        if len(bridges) == 0:
    ...
    async def validate_enocean_conf(self, user_input) -> bool:
        ...
        path_is_valid = await self.opp.async_add_executor_job(
