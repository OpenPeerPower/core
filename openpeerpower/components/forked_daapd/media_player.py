"""This library brings support for forked_daapd to Open Peer Power."""
import asyncio
...
from openpeerpower.components.media_player.const import MEDIA_TYPE_MUSIC
...
class ForkedDaapdUpdater:
    ...
    def _disconnected_callback(self):
        async_dispatcher_send(
            self.opp, SIGNAL_UPDATE_MASTER.format(self._entry_id), False
        )
        async_dispatcher_send(
            self.opp, SIGNAL_UPDATE_OUTPUTS.format(self._entry_id), []
        )
    ...
    async def _update(self, update_types):
        ...
        if update_events:
            ...
            async_dispatcher_send(
                self.opp, SIGNAL_UPDATE_MASTER.format(self._entry_id), True
            )
