...
async def async_modbus_setup(
        ...
        for component, conf_key in PLATFORMS:
            if conf_key in conf_hub:
                opp.async_create_task(
                    async_load_platform(opp, component, DOMAIN, conf_hub, config)
                )
    ...
    opp.bus.async_listen_once(EVENT_OPENPEERPOWER_STOP, async_stop_modbus)
...
class ModbusHub:
    ...
    def __init__(self, opp, client_config):
    ...
    async def async_setup(self):
        ...
        async with self._lock:
            await self.opp.async_add_executor_job(self._pymodbus_connect)
        ...
        if self._config_delay:
            self._async_cancel_listener = async_call_later(
                self.opp, self._config_delay, self.async_end_delay
            )
    ...
    async def async_close(self):
        ...
        async with self._lock:
            return await self.opp.async_add_executor_job(self._pymodbus_close)
