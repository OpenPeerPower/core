...
async def async_setup(opp, config):
    ...
    async def async_scan_service(service):
        ...
        for entity in image_entities:
            ...
            update_tasks.append(asyncio.create_task(entity.async_update_op_state(True)))
...
class ImageProcessingEntity(Entity):
    ...
    async def async_process_image(self, image):
        """Process image."""
        return await self.opp.async_add_executor_job(self.process_image, image)
    ...
    async def async_update(self):
        ...
        """
        camera = self.opp.components.camera
        image = None
...
class ImageProcessingFaceEntity(ImageProcessingEntity):
    ...
    def process_faces(self, faces, total):
        ...
        run_callback_threadsafe(
            self.opp.loop, self.async_process_faces, faces, total
        ).result()
    ...
    def async_process_faces(self, faces, total):
        ...
        for face in faces:
            ...
            self.opp.async_add_job(self.opp.bus.async_fire, EVENT_DETECT_FACE, face)
