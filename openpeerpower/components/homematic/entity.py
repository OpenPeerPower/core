...
class HMDevice(Entity):
    ...
    def update(self):
        ...
        # Initialize
        self._homematic = self.opp.data[DATA_HOMEMATIC]
...
class HMHub(Entity):
    ...
    def __init__(self, opp, homematic, name):
        ...
        # Load data
        self.opp.helpers.event.track_time_interval(self._update_hub, SCAN_INTERVAL_HUB)
        self.opp.add_job(self._update_hub, None)

        self.opp.helpers.event.track_time_interval(
            self._update_variables, SCAN_INTERVAL_VARIABLES
        )
        self.opp.add_job(self._update_variables, None)
