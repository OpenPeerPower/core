...
from openpeerpower.components.camera import ATTR_ENTITY_ID, SUPPORT_ON_OFF, Camera
...
class LogiCam(Camera):
    ...
    async def download_livestream(self, filename, duration):
        ...
        if not self.opp.config.is_allowed_path(stream_file):
    ...
    async def livestream_snapshot(self, filename):
        ...
        # Respect configured allowed paths.
        if not self.opp.config.is_allowed_path(snapshot_file):
