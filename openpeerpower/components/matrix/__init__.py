...
from openpeerpower.components.notify import ATTR_DATA, ATTR_MESSAGE, ATTR_TARGET
...
class MatrixBot:
    ...
    def _handle_room_message(self, room_id, room, event):
        ...
        if event["content"]["body"][0] == "!":
            ...
            if command:
                ...
                self.opp.bus.fire(EVENT_MATRIX_COMMAND, event_data)
    ...
    def _send_image(self, img, target_rooms):
        ...
        if not self.opp.config.is_allowed_path(img):
