...
from openpeerpower.helpers.typing import ServiceCallType
...
async def async_setup(opp, config):
    ...
    try:
        knx_module = KNXModule(opp, config)
        opp.data[DOMAIN] = knx_module
    ...
    if CONF_KNX_EXPOSE in config[DOMAIN]:
        for expose_config in config[DOMAIN][CONF_KNX_EXPOSE]:
            knx_module.exposures.append(
                create_knx_exposure(opp, knx_module.xknx, expose_config)
            )
...
class KNXModule:
    ...
    def config_file(self):
        ...
        if not config_file.startswith("/"):
            return self.opp.config.path(config_file)
        return config_file
    ...
    async def service_exposure_register_modify(self, call):
        ...
        exposure = create_knx_exposure(self.opp, self.xknx, call.data)
