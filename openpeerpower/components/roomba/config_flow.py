...
class RoombaConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input=None):
        ...
        async with self.opp.data.setdefault(ROOMBA_DISCOVERY_LOCK, asyncio.Lock()):
            devices = await self.opp.async_add_executor_job(discovery.get_all)
    ...
    async def async_step_link(self, user_input=None):
        ...
        try:
            password = await self.opp.async_add_executor_job(
        ...
        if not self.name:
            ...
            await async_disconnect_or_timeout(self.opp, info[ROOMBA_SESSION])
