...
from openpeerpower.components.dhcp import HOSTNAME, IP_ADDRESS
...
AUTH_HELP_URL_VALUE = "https://www.openpeerpower.io/integrations/roomba/#manually-retrieving-your-credentials"
...
class RoombaConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input=None):
        ...
        devices = await _async_discover_roombas(self.opp, self.host)
    ...
    async def async_step_link(self, user_input=None):
        ...
        try:
            password = await self.opp.async_add_executor_job(roomba_pw.get_password)
        ...
        if not self.name:
            try:
                info = await validate_input(self.opp, config)
...
async def _async_discover_roombas(opp, host):
    discovered_hosts = set()
    devices = []
    discover_lock = opp.data.setdefault(ROOMBA_DISCOVERY_LOCK, asyncio.Lock())
    ...
    for attempt in range(discover_attempts + 1):
        async with discover_lock:
            ...
            try:
                if host:
                    device = await opp.async_add_executor_job(discovery.get, host)
                    discovered = [device] if device else []
                else:
                    discovered = await opp.async_add_executor_job(discovery.get_all)
            except OSError:
