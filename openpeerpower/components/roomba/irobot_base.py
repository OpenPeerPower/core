...
class IRobotVacuum(IRobotEntity, StateVacuumEntity):
    ...
    def device_state_attributes(self):
        ...
        if self.state == STATE_CLEANING:
            ...
            if cleaned_area and self.opp.config.units.is_metric:
    ...
    async def async_start(self):
        ...
        if self.state == STATE_PAUSED:
            await self.opp.async_add_executor_job(self.vacuum.send_command, "resume")
        else:
            await self.opp.async_add_executor_job(self.vacuum.send_command, "start")

    async def async_stop(self, **kwargs):
        """Stop the vacuum cleaner."""
        await self.opp.async_add_executor_job(self.vacuum.send_command, "stop")

    async def async_pause(self):
        """Pause the cleaning cycle."""
        await self.opp.async_add_executor_job(self.vacuum.send_command, "pause")
    ...
    async def async_return_to_base(self, **kwargs):
        ...
        await self.opp.async_add_executor_job(self.vacuum.send_command, "dock")

    async def async_locate(self, **kwargs):
        """Located vacuum."""
        await self.opp.async_add_executor_job(self.vacuum.send_command, "find")
