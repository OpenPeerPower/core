...
async def async_setup_entry(
    ...
    if not config_entry.options:
        ...
        if CONF_UNITS not in options:
            options[CONF_UNITS] = opp.config.units.name
...
async def async_setup_platform(
    opp: OpenPeerPower, config, add_entities_callback, discovery_info=None
):
...
class GoogleTravelTimeSensor(SensorEntity):
    ...
    def update(self):
        ...
        self._destination = resolve_zone(self.opp, self._destination)
        self._origin = resolve_zone(self.opp, self._origin)
