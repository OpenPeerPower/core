...
from openpeerpower.const import CONF_API_KEY, CONF_MODE, CONF_NAME
...
def is_dupe_import(
    opp: OpenPeerPower, entry: config_entries.ConfigEntry, user_input: dict[str, Any]
) -> bool:
    ...
    units = options.get(CONF_UNITS) or opp.config.units.name
    ...
    async def async_step_user(self, user_input=None):
        ...
        if user_input:
            ...
            if self.source == config_entries.SOURCE_IMPORT and any(
                is_dupe_import(self.opp, entry, user_input)
            ...
            if (
                ...
                or await self.opp.async_add_executor_job(
