...
from openpeerpower.core import Context, CoreState, OpenPeerPower, State, callback
from openpeerpower.helpers.area_registry import AreaEntry
...
async def _get_area(opp, entity_entry, device_entry) -> AreaEntry | None:
...
class AbstractConfig(ABC):
    ...
    async def async_initialize(self):
        ...
        self._store = GoogleConfigStore(self.opp)
    ...
    def async_enable_report_state(self):
        ...
        if self._unsub_report_state is None:
            self._unsub_report_state = async_enable_report_state(self.opp, self)
...
class GoogleEntity:
    ...
    def __init__(
        self, opp: OpenPeerPower, config: AbstractConfig, state: State
    ) -> None:
    ...
    def traits(self):
        ...
        self._traits = [
            Trait(self.opp, state, self.config)
