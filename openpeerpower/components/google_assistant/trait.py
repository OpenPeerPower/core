...
from openpeerpower.components.media_player.const import MEDIA_TYPE_CHANNEL
...
class CameraStreamTrait(_Trait):
    ...
    async def execute(self, command, data, params, challenge):
        ...
        url = await self.opp.components.camera.async_request_stream(
            self.state.entity_id, "hls"
        )
        self.stream_info = {
            "cameraStreamAccessUrl": f"{get_url(self.opp)}{url}",
            "cameraStreamReceiverAppId": CAST_APP_ID_OPENPEERPOWER,
        }
    ...
    def sync_attributes(self):
        ...
        return {
            "temperatureUnitForUX": _google_temp_unit(
                self.opp.config.units.temperature_unit
            ),
