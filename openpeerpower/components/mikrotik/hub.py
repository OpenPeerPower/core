...
from openpeerpower.const import CONF_HOST, CONF_PASSWORD, CONF_USERNAME, CONF_VERIFY_SSL
...
class MikrotikHub:
    ...
    async def request_update(self):
        ...
        self.progress = self.opp.async_create_task(self.async_update())
        await self.progress
    ...
    async def async_update(self):
        ...
        await self.opp.async_add_executor_job(self._mk_data.update)
        async_dispatcher_send(self.opp, self.signal_update)

    async def async_setup(self):
        """Set up the Mikrotik hub."""
        try:
            api = await self.opp.async_add_executor_job(
                get_api, self.opp, self.config_entry.data
            )
        ...
        self._mk_data = MikrotikData(self.opp, self.config_entry, api)
        await self.async_add_options()
        await self.opp.async_add_executor_job(self._mk_data.get_hub_details)
        await self.opp.async_add_executor_job(self._mk_data.update)
