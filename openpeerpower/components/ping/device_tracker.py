...
class HostSubProcess:
    ...
    def __init__(self, ip_address, dev_id, opp, config, privileged):
...
async def async_setup_scanner(opp, config, async_see, discovery_info=None):
    ...
    if privileged is None:
        hosts = [
            HostSubProcess(ip, dev_id, opp, config, privileged)
        ...
        async def async_update(now):
            ...
            results = await gather_with_concurrency(
                ...
                *[opp.async_add_executor_job(host.update) for host in hosts],
            )
        ...
        async def async_update(now):
            """Update all the hosts on every interval time."""
            responses = await opp.async_add_executor_job(
                partial(
                    multiping,
                    ip_to_dev_id.keys(),
                    ...
                    id=async_get_next_ping_id(opp, len(ip_to_dev_id)),
                )
            )
