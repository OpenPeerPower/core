...
from openpeerpower.core import OpenPeerPower, split_entity_id
import openpeerpower.helpers.config_validation as cv
from openpeerpower.helpers.storage import STORAGE_DIR
import openpeerpower.util.temperature as temp_util
...
def show_setup_message(opp, entry_id, bridge_name, pincode, uri):
    ...
    opp.components.persistent_notification.create(message, "HomeKit Pairing", entry_id)
...
def accessory_friendly_name(opp_name, accessory):
    """Return the combined name for the accessory.

    The mDNS name and the Open Peer Power config entry
