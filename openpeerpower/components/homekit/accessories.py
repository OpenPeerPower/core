...
from openpeerpower.components.remote import SUPPORT_ACTIVITY
...
def get_accessory(opp, driver, state, aid, config):  # noqa: C901
...
class HomeAccessory(Accessory):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        entity_attributes = self.opp.states.get(self.entity_id).attributes
    ...
    async def run(self):
        ...
        self._subscriptions.append(
            async_track_state_change_event(
                self.opp, [self.entity_id], self.async_update_event_state_callback
            )
        )
...
class HomeDriver(AccessoryDriver):
    ...
    def pair(self, client_uuid, client_public):
        ...
        if success:
            dismiss_setup_message(self.opp, self._entry_id)
        return success
