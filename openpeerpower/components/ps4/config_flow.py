...
class PlayStation4FlowHandler(config_entries.ConfigFlow):
    ...
    async def async_step_user(self, user_input=None):
        ...
        ports = PORT_MSG.keys()
        failed = await self.opp.async_add_executor_job(self.helper.port_bind, ports)
        if failed in ports:
    ...
    async def async_step_creds(self, user_input=None):
        ...
        if user_input is not None:
            try:
                self.creds = await self.opp.async_add_executor_job(
    ...
    async def async_step_link(self, user_input=None):
        ...
        if user_input is None:
            ...
            entries = self.opp.config_entries.async_entries(DOMAIN)
            if entries:
        ...
        if user_input is not None:
            ...
            is_ready, is_login = await self.opp.async_add_executor_job(
                self.helper.link,
                self.host,
                self.creds,
                self.pin,
