...
from openpeerpower.components.ps4 import format_unique_id, load_games, save_games
...
class PS4Device(MediaPlayerEntity):
    ...
    def subscribe_to_protocol(self):
        ...
        self.opp.data[PS4_DATA].protocol.add_callback(self._ps4, self.status_callback)

    @callback
    def unsubscribe_to_protocol(self):
        """Notify protocol to remove callback."""
        self.opp.data[PS4_DATA].protocol.remove_callback(
    ...
    async def async_update(self):
        ...
        if self._ps4.ddp_protocol is None:
            # Use socket.socket.
            await self.opp.async_add_executor_job(self._ps4.get_status)
            if self._info is None:
                # Add entity to registry.
                await self.async_get_device_info(self._ps4.status)
            self._ps4.ddp_protocol = self.opp.data[PS4_DATA].protocol
    ...
    def add_games(self, title_id, app_name, image, g_type, is_locked=False):
        ...
        if title_id is not None and title_id not in games:
            ...
            games.update(game)
            save_games(self.opp, games, self._unique_id)
    ...
    async def async_get_device_info(self, status):
        ...
        if status is None:
            ...
            e_registry = await entity_registry.async_get_registry(self.opp)
            d_registry = await device_registry.async_get_registry(self.opp)
    ...
    async def async_will_remove_from_opp(self):
        ...
        self.opp.data[PS4_DATA].devices.remove(self)
