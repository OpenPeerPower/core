...
def load_games(opp: OpenPeerPower, unique_id: str) -> dict:
...
def save_games(opp: OpenPeerPower, games: dict, unique_id: str):
    """Save games to file."""
    g_file = opp.config.path(GAMES_FILE.format(unique_id))
    try:
...
def _reformat_data(opp: OpenPeerPower, games: dict, unique_id: str) -> dict:
...
def service_handle(opp: OpenPeerPower):
    """Handle for services."""
