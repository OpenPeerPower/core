...
class LutronCasetaFlowHandler(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_link(self, user_input=None):
        ...
        if (
            ...
            and await self.opp.async_add_executor_job(self._tls_assets_exist)
        ...
        if user_input is not None:
            ...
            if not errors:
                await self.opp.async_add_executor_job(self._write_tls_assets, assets)
                return self.async_create_entry(title=self.bridge_id, data=self.data)
    ...
    def _tls_assets_exist(self):
        ...
        for conf_key in FILE_MAPPING.values():
            if not os.path.exists(self.opp.config.path(self.data[conf_key])):
                return False
        return True
    ...
    async def async_validate_connectable_bridge_config(self):
        ...
        try:
            bridge = Smartbridge.create_tls(
                ...
                keyfile=self.opp.config.path(self.data[CONF_KEYFILE]),
                certfile=self.opp.config.path(self.data[CONF_CERTFILE]),
                ca_certs=self.opp.config.path(self.data[CONF_CA_CERTS]),
            )
        except ssl.SSLError:
            _LOGGER.error(
