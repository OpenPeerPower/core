...
from openpeerpower.const import CONF_ENTITY_ID, CONF_MODE, CONF_NAME
...
class ProxyCamera(Camera):
    ...
    def camera_image(self):
        ...
        return asyncio.run_coroutine_threadsafe(
            self.async_camera_image(), self.opp.loop
        ).result()
    ...
    async def async_camera_image(self):
        ...
        image = await async_get_image(self.opp, self._proxied_camera)
        if not image:
        ...
        if self._mode == MODE_RESIZE:
            ...
            job = _crop_image
        image = await self.opp.async_add_executor_job(
