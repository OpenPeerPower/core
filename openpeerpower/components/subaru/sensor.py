...
from openpeerpower.components.sensor import DEVICE_CLASSES, SensorEntity
...
from openpeerpower.util.distance import convert as dist_convert
from openpeerpower.util.unit_system import (
    IMPERIAL_SYSTEM,
    LENGTH_UNITS,
    PRESSURE_UNITS,
    TEMPERATURE_UNITS,
)
from openpeerpower.util.volume import convert as vol_convert
...
class SubaruSensor(SubaruEntity, SensorEntity):
    ...
    def state(self):
        ...
        if self.api_unit in TEMPERATURE_UNITS:
            return round(
                self.opp.config.units.temperature(self.current_value, self.api_unit), 1
            )
    ...
    def unit_of_measurement(self):
        ...
        if self.api_unit in LENGTH_UNITS:
            return self.opp.config.units.length_unit

        if self.api_unit in PRESSURE_UNITS:
            if self.opp.config.units == IMPERIAL_SYSTEM:
                return self.opp.config.units.pressure_unit
            return PRESSURE_HPA
