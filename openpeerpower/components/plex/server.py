...
from openpeerpower.const import CONF_CLIENT_ID, CONF_TOKEN, CONF_URL, CONF_VERIFY_SSL
...
class PlexServer:
    ...
    async def async_update_session(self, payload):
        ...
        if active_session.media_content_id != rating_key and state in [
            ...
            await self.opp.async_add_executor_job(update_with_new_media)
    ...
    async def _async_update_platforms(self):  # noqa: C901
        ...
        await self.opp.data[DOMAIN][GDM_DEBOUNCER]()
        ...
        try:
            devices, sessions, plextv_clients = await self.opp.async_add_executor_job(
            ...
            await self.opp.config_entries.async_reload(self.entry_id)
            return
        except (
        ...
        def connect_new_clients():
            ...
            for gdm_entry in self.opp.data[DOMAIN][GDM_SCANNER].entries:
        ...
        await self.opp.async_add_executor_job(sync_tasks)
