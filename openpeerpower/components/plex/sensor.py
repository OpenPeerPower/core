...
async def async_setup_entry(opp, config_entry, async_add_entities):
    ...
    sensors = [PlexSensor(opp, plexserver)]

    def create_library_sensors():
        """Create Plex library sensors with sync calls."""
        for library in plexserver.library.sections():
            sensors.append(PlexLibrarySectionSensor(opp, plexserver, library))

    await opp.async_add_executor_job(create_library_sensors)
...
class PlexLibrarySectionSensor(SensorEntity):
    ...
    def __init__(self, opp, plex_server, plex_library_section):
        """Initialize the sensor."""
        self._server = plex_server
    ...
    async def async_refresh_sensor(self):
        ...
        try:
            await self.opp.async_add_executor_job(self._update_state_and_attrs)
            self._available = True
        except NotFound:
