...
from openpeerpower.const import VOLT
...
class ElkSensor(ElkAttachedEntity):
    ...
    async def async_counter_refresh(self):
        ...
        if not isinstance(self, ElkCounter):
            raise OpenPeerPowerError("supported only on ElkM1 Counter sensors")
        self._element.get()
    ...
    async def async_zone_bypass(self, code=None):
        ...
        if not isinstance(self, ElkZone):
            raise OpenPeerPowerError("supported only on ElkM1 Zone sensors")
