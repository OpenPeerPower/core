...
async def async_setup_entry(opp, config_entry):
    ...
    for service_name, service in SERVICES:
        if not opp.services.has_service(DOMAIN, service_name):
...
async def async_unload_entry(opp, config_entry):
    ...
    if unload_ok and config_entry.entry_id in opp.data[DOMAIN]:
        host = opp.data[DOMAIN].pop(config_entry.entry_id)
        await host[CONNECTION].async_close()

    # unregister service calls
    if unload_ok and not opp.data[DOMAIN]:  # check if this is the last entry to unload
