...
class TotalConnectConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input=None):
        ...
        if user_input is not None:
            ...
            client = await self.opp.async_add_executor_job(
        ...
        if user_entry is not None:
            for location_id in self.usercodes:
                if self.usercodes[location_id] is None:
                    valid = await self.opp.async_add_executor_job(
    ...
    async def async_step_reauth_confirm(self, user_input=None):
        ...
        self.opp.config_entries.async_update_entry(existing_entry, data=new_entry)
