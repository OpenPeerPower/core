...
from openpeerpower.const import ATTR_COMMAND, ATTR_NAME
...
async def websocket_reconfigure_node(opp, connection, msg):
    ...
    zha_gateway: ZhaGatewayType = opp.data[DATA_ZHA][DATA_ZHA_GATEWAY]
    ieee = msg[ATTR_IEEE]
    ...
    remove_dispatcher_function = async_dispatcher_connect(
        opp, ZHA_CHANNEL_MSG, forward_messages
    )
...
async def websocket_get_configuration(opp, connection, msg):
...
async def websocket_update_zha_configuration(opp, connection, msg):
    ...
    status = await opp.config_entries.async_reload(zha_gateway.config_entry.entry_id)
...
def async_load_api(opp):
    ...
    websocket_api.async_register_command(opp, websocket_get_configuration)
    websocket_api.async_register_command(opp, websocket_update_zha_configuration)
