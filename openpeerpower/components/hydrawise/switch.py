...
class HydrawiseSwitch(HydrawiseEntity, SwitchEntity):
    ...
    def turn_on(self, **kwargs):
        ...
        if self._sensor_type == "manual_watering":
            self.opp.data[DATA_HYDRAWISE].data.run_zone(
                self._default_watering_timer, relay_data
            )
        elif self._sensor_type == "auto_watering":
            self.opp.data[DATA_HYDRAWISE].data.suspend_zone(0, relay_data)

    def turn_off(self, **kwargs):
        """Turn the device off."""
        relay_data = self.data["relay"] - 1
        if self._sensor_type == "manual_watering":
            self.opp.data[DATA_HYDRAWISE].data.run_zone(0, relay_data)
        elif self._sensor_type == "auto_watering":
            self.opp.data[DATA_HYDRAWISE].data.suspend_zone(365, relay_data)
