...
async def async_setup_entry(
    ...
    if DATA_CONDITION not in opp.data[DOMAIN]:
        cond_keys = [str(chr(x)) for x in range(97, 123)]
        opp.data[DOMAIN][DATA_CONDITION] = dict.fromkeys(cond_keys)
        for cond, condlst in CONDITION_CLASSES.items():
            for condi in condlst:
                opp.data[DOMAIN][DATA_CONDITION][condi] = cond
...
class BrWeather(WeatherEntity):
    ...
    def condition(self):
        ...
        if self._data and self._data.condition:
            ...
            if ccode:
                conditions = self.opp.data[DOMAIN].get(DATA_CONDITION)
                if conditions:
    ...
    def forecast(self):
        ...
        fcdata_out = []
        cond = self.opp.data[DOMAIN][DATA_CONDITION]
