...
from openpeerpower.components.plex.services import play_on_sonos
...
from openpeerpower.helpers.network import is_internal_request
...
class SonosEntity(MediaPlayerEntity):
    ...
    async def async_added_to_opp(self):
        ...
        self.opp.data[DATA_SONOS].entities.append(self)
        ...
        self.opp.async_add_executor_job(_rebuild_groups)
    ...
    async def async_seen(self, player):
        ...
        done = await self.opp.async_add_executor_job(self._attach_player)
        if not done:
            self._seen_timer()
            self.async_unseen()
    ...
    def async_unseen(self, now=None):
        ...
        self.opp.async_add_executor_job(_unsub, self._subscriptions)
    ...
    def update_groups(self, event=None):
        ...
        def _async_regroup(group):
            ...
            sonos_group = []
            for uid in group:
                entity = _get_entity_from_soco_uid(self.opp, uid)
                if entity:
            ...
            for slave_uid in group[1:]:
                ...
                if slave:
                    ...
                    slave.async_schedule_update_op_state()
    ...
    def play_media(self, media_type, media_id, **kwargs):
        ...
        if media_id and media_id.startswith(PLEX_URI_SCHEME):
            ...
            play_on_sonos(self.opp, media_type, media_id, self.name)
    ...
    async def async_get_browse_image(
        ...
        if (
            ...
            item = await self.opp.async_add_executor_job(
                get_media,
                self._media_library,
                media_content_id,
    ...
    async def async_browse_media(self, media_content_type=None, media_content_id=None):
        ...
        is_internal = is_internal_request(self.opp)
        ...
        response = await self.opp.async_add_executor_job(
