...
from openpeerpower.const import ATTR_TIME, STATE_IDLE, STATE_PAUSED, STATE_PLAYING
...
async def async_setup_entry(
    ...
    async def async_service_handle(service_call: ServiceCall) -> None:
        ...
        if service_call.service == SERVICE_JOIN:
            ...
            if master:
                await SonosSpeaker.join_multi(opp, master.speaker, speakers)  # type: ignore[arg-type]
            else:
                _LOGGER.error(
            ...
            await SonosSpeaker.unjoin_multi(opp, speakers)  # type: ignore[arg-type]
        elif service_call.service == SERVICE_SNAPSHOT:
            await SonosSpeaker.snapshot_multi(
                opp, speakers, service_call.data[ATTR_WITH_GROUP]  # type: ignore[arg-type]
            )
    ...
    config_entry.async_on_unload(
        async_dispatcher_connect(opp, SONOS_CREATE_MEDIA_PLAYER, async_create_entities)
    )
...
class SonosMediaPlayerEntity(SonosEntity, MediaPlayerEntity):
    ...
    async def async_update(self) -> None:
        ...
        await self.opp.async_add_executor_job(self._update)
