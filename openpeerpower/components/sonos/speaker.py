...
from openpeerpower.helpers import entity_registry as ent_reg
...
class SonosSpeaker:
    ...
    def __init__(
        self, opp: OpenPeerPower, soco: SoCo, speaker_info: dict[str, Any]
    ) -> None:
    ...
    def setup(self) -> None:
        ...
        self._seen_dispatcher = dispatcher_connect(
            self.opp, f"{SONOS_SEEN}-{self.soco.uid}", self.async_seen
        )
        ...
        if (battery_info := fetch_battery_info_or_none(self.soco)) is None:
            ...
            if battery_info:
                ...
                self._battery_poll_timer = self.opp.helpers.event.track_time_interval(
            ...
            dispatcher_send(self.opp, SONOS_CREATE_BATTERY, self)

        if new_alarms := self.update_alarms_for_speaker():
            dispatcher_send(self.opp, SONOS_CREATE_ALARM, self, new_alarms)
        else:
        ...
        dispatcher_send(self.opp, SONOS_CREATE_MEDIA_PLAYER, self)
    ...
    def write_entity_states(self) -> None:
        ...
        dispatcher_send(self.opp, f"{SONOS_STATE_UPDATED}-{self.soco.uid}")

    @callback
    def async_write_entity_states(self) -> None:
        """Write states for associated SonosEntity instances."""
        async_dispatcher_send(self.opp, f"{SONOS_STATE_UPDATED}-{self.soco.uid}")
    ...
    def favorites(self) -> SonosFavorites:
        ...
        return self.opp.data[DATA_SONOS].favorites[self.household_id]
    ...
    def processed_alarm_events(self) -> deque[str]:
        ...
        return self.opp.data[DATA_SONOS].processed_alarm_events
    ...
    async def async_subscribe(self) -> bool:
        ...
        try:
            await self.opp.async_add_executor_job(self.set_basic_info)
    ...
    def async_renew_failed(self, exception: Exception) -> None:
        ...
        self.opp.async_create_task(self.async_resubscribe(exception))
    ...
    def async_dispatch_alarms(self, event: SonosEvent) -> None:
        ...
        self.opp.async_add_executor_job(self.update_alarms)

    @callback
    def async_dispatch_device_properties(self, event: SonosEvent) -> None:
        """Update device properties from an event."""
        self.opp.async_create_task(self.async_update_device_properties(event))
    ...
    def async_dispatch_media_update(self, event: SonosEvent) -> None:
        ...
        self.opp.async_add_executor_job(self.update_media, event)
    ...
    async def async_seen(self, soco: SoCo | None = None) -> None:
        ...
        self._seen_timer = self.opp.helpers.event.async_call_later(
        ...
        self._poll_timer = self.opp.helpers.event.async_track_time_interval(
            partial(
                async_dispatcher_send,
                self.opp,
    ...
    async def async_unseen(self, now: datetime.datetime | None = None) -> None:
        ...
        self._subscriptions = []
        self.opp.data[DATA_SONOS].ssdp_known.remove(self.soco.uid)
    ...
    def update_alarms_for_speaker(self) -> set[str]:
        ...
        Updates opp.data[DATA_SONOS].alarms and returns a list of all alarms that are new.
        """
        new_alarms = set()
        stored_alarms = self.opp.data[DATA_SONOS].alarms
    ...
    def update_alarms(self) -> None:
        ...
        if new_alarms := self.update_alarms_for_speaker():
            dispatcher_send(self.opp, SONOS_CREATE_ALARM, self, new_alarms)
        dispatcher_send(self.opp, SONOS_ALARM_UPDATE)
    ...
    async def async_update_battery_info(self, battery_dict: dict[str, Any]) -> None:
        ...
        if is_charging == self.charging:
            ...
            if battery_info := await self.opp.async_add_executor_job(
    ...
    def update_groups(self, event: SonosEvent | None = None) -> None:
        ...
        if coro:
            self.opp.add_job(coro)  # type: ignore
    ...
    def async_update_groups(self, event: SonosEvent | None = None) -> None:
        ...
        coro = self.create_update_groups_coro(event)
        if coro:
            self.opp.async_add_job(coro)  # type: ignore
    ...
    def create_update_groups_coro(
        ...
        async def _async_extract_group(event: SonosEvent) -> list[str]:
            ...
            return await self.opp.async_add_executor_job(_get_soco_group)

        @callback
        def _async_regroup(group: list[str]) -> None:
            """Rebuild internal group layout."""
            entity_registry = ent_reg.async_get(self.opp)
            sonos_group = []
            sonos_group_entities = []

            for uid in group:
                speaker = self.opp.data[DATA_SONOS].discovered.get(uid)
                if speaker:
            ...
            for slave_uid in group[1:]:
                slave = self.opp.data[DATA_SONOS].discovered.get(slave_uid)
                if slave:
        ...
        async def _async_handle_group_event(event: SonosEvent) -> None:
            ...
            async with self.opp.data[DATA_SONOS].topology_condition:
                group = await _async_extract_group(event)

                if self.soco.uid == group[0]:
                    _async_regroup(group)

                    self.opp.data[DATA_SONOS].topology_condition.notify_all()
    ...
    async def join_multi(
        ...
        async with opp.data[DATA_SONOS].topology_condition:
            group: list[SonosSpeaker] = await opp.async_add_executor_job(
                master.join, speakers
            )
            await SonosSpeaker.wait_for_groups(opp, [group])
    ...
    @staticmethod
    async def unjoin_multi(opp: OpenPeerPower, speakers: list[SonosSpeaker]) -> None:
        ...
        async with opp.data[DATA_SONOS].topology_condition:
            await opp.async_add_executor_job(_unjoin_all, speakers)
            await SonosSpeaker.wait_for_groups(opp, [[s] for s in speakers])
    ...
    async def snapshot_multi(
        opp: OpenPeerPower, speakers: list[SonosSpeaker], with_group: bool
    ) -> None:
        ...
        async with opp.data[DATA_SONOS].topology_condition:
            await opp.async_add_executor_job(_snapshot_all, speakers_set)
    ...
    async def restore_multi(
        ...
        def _restore_groups(
            ...
            for speaker in (s for s in speakers if s.is_coordinator):
                if speaker.media.playback_status == SONOS_STATE_PLAYING:
                    opp.async_create_task(speaker.soco.pause())
        ...
        async with opp.data[DATA_SONOS].topology_condition:
            ...
            await SonosSpeaker.wait_for_groups(opp, groups)
            await opp.async_add_executor_job(_restore_players, speakers_set)

    @staticmethod
    async def wait_for_groups(
        opp: OpenPeerPower, groups: list[list[SonosSpeaker]]
    ) -> None:
        ...
        for speaker in opp.data[DATA_SONOS].discovered.values():
    ...
    def update_media(self, event: SonosEvent | None = None) -> None:
        ...
        # Also update slaves
        speakers = self.opp.data[DATA_SONOS].discovered.values()
        for speaker in speakers:
