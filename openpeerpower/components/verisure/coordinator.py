...
from openpeerpower.const import CONF_EMAIL, CONF_PASSWORD, HTTP_SERVICE_UNAVAILABLE
...
class VerisureDataUpdateCoordinator(DataUpdateCoordinator):
    ...
    def __init__(self, opp: OpenPeerPower, entry: ConfigEntry) -> None:
        ...
        self.verisure = Verisure(
            ...
            cookieFileName=opp.config.path(STORAGE_DIR, f"verisure_{entry.entry_id}"),
        )

        super().__init__(
            opp, LOGGER, name=DOMAIN, update_interval=DEFAULT_SCAN_INTERVAL
        )

    async def async_login(self) -> bool:
        """Login to Verisure."""
        try:
            await self.opp.async_add_executor_job(self.verisure.login)
    ...
    async def async_logout(self, _event: Event) -> bool:
        ...
        try:
            await self.opp.async_add_executor_job(self.verisure.logout)
        except VerisureError as ex:
    ...
    async def _async_update_data(self) -> dict:
        ...
        # Store data in a way Open Peer Power can easily consume it
        return {
