...
class EnvisalinkAlarm(EnvisalinkDevice, AlarmControlPanelEntity):
    ...
    async def async_added_to_opp(self):
        ...
        self.async_on_remove(
            async_dispatcher_connect(
                self.opp, SIGNAL_KEYPAD_UPDATE, self._update_callback
            )
        )
        self.async_on_remove(
            async_dispatcher_connect(
    ...
    async def async_alarm_disarm(self, code=None):
        """Send disarm command."""
        if code:
            self.opp.data[DATA_EVL].disarm_partition(str(code), self._partition_number)
        else:
            self.opp.data[DATA_EVL].disarm_partition(
    ...
    async def async_alarm_arm_home(self, code=None):
        ...
        if code:
            self.opp.data[DATA_EVL].arm_stay_partition(
    ...
    async def async_alarm_arm_away(self, code=None):
        ...
        if code:
            self.opp.data[DATA_EVL].arm_away_partition(
                str(code), self._partition_number
            )
        else:
            self.opp.data[DATA_EVL].arm_away_partition(
    ...
    async def async_alarm_trigger(self, code=None):
        ...
        self.opp.data[DATA_EVL].panic_alarm(self._panic_type)

    async def async_alarm_arm_night(self, code=None):
        """Send arm night command."""
        self.opp.data[DATA_EVL].arm_night_partition(
    ...
    def async_alarm_keypress(self, keypress=None):
        ...
        if keypress:
            self.opp.data[DATA_EVL].keypresses_to_partition(
