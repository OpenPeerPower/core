...
class KodiEntity(MediaPlayerEntity):
    ...
    async def async_added_to_opp(self):
        ...
        if self.opp.state == CoreState.running:
            ...
            self.opp.bus.async_listen_once(EVENT_OPENPEERPOWER_STARTED, start_watchdog)
    ...
    async def async_turn_on(self):
        ...
        self.opp.bus.async_fire(EVENT_TURN_ON, {ATTR_ENTITY_ID: self.entity_id})

    async def async_turn_off(self):
        """Turn the media player off."""
        _LOGGER.debug("Firing event to turn off device")
        self.opp.bus.async_fire(EVENT_TURN_OFF, {ATTR_ENTITY_ID: self.entity_id})
