...
class LocalSource(MediaSource):
    ...
    def async_full_path(self, source_dir_id, location) -> Path:
        """Return full path."""
        return Path(self.opp.config.media_dirs[source_dir_id], location)
    ...
    def async_parse_identifier(self, item: MediaSourceItem) -> tuple[str, str]:
        ...
        if source_dir_id not in self.opp.config.media_dirs:
    ...
    async def async_resolve_media(self, item: MediaSourceItem) -> str:
        ...
        if source_dir_id == "" or source_dir_id not in self.opp.config.media_dirs:
            raise Unresolvable("Unknown source directory.")
    ...
    def _browse_media(self, source_dir_id: str, location: Path):
        ...
        if source_dir_id == "" and len(self.opp.config.media_dirs) == 1:
            source_dir_id = list(self.opp.config.media_dirs)[0]
        ...
        if source_dir_id == "":
            ...
            base.children = [
                ...
                for source_dir_id in self.opp.config.media_dirs
            ]

            return base

        full_path = Path(self.opp.config.media_dirs[source_dir_id], location)
    ...
    def _build_item_response(self, source_dir_id: str, path: Path, is_child=False):
        ...
        media = BrowseMediaSource(
            domain=DOMAIN,
            identifier=f"{source_dir_id}/{path.relative_to(self.opp.config.media_dirs[source_dir_id])}",
...
class LocalMediaView(OpenPeerPowerView):
    """
    ...
    def __init__(self, opp: OpenPeerPower, source: LocalSource) -> None:
