...
from openpeerpower.helpers.typing import ConfigType, DiscoveryInfoType, EventType
...
class MediaGroup(MediaPlayerEntity):
    ...
    async def async_added_to_opp(self) -> None:
        ...
        async_track_state_change_event(
            self.opp, self._entities, self.async_on_state_change
        )
        self.async_update_state()
    ...
    async def async_volume_up(self) -> None:
        ...
        for entity in self._features[KEY_VOLUME]:
            volume_level = self.opp.states.get(entity).attributes["volume_level"]  # type: ignore
            if volume_level < 1:
    ...
    def async_update_state(self) -> None:
        ...
        states = [self.opp.states.get(entity) for entity in self._entities]
