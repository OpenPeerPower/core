...
class AppleTVEntity(Entity):
    ...
    async def async_added_to_opp(self):
        ...
        self.async_on_remove(
            async_dispatcher_connect(
                self.opp, f"{SIGNAL_CONNECTED}_{self._identifier}", _async_connected
            )
        )
        self.async_on_remove(
            async_dispatcher_connect(
...
class AppleTVManager:
    ...
    async def _scan(self):
        ...
        atvs = await scan(
            self.opp.loop, identifier=identifier, protocol=protocol, hosts=[address]
        )
        if atvs:
            return atvs[0]
        ...
        atvs = await scan(self.opp.loop, identifier=identifier, protocol=protocol)
        if atvs:
            return atvs[0]
    ...
    async def _connect(self, conf):
        ...
        self.atv = await connect(conf, self.opp.loop, session=session)
    ...
    def _dispatch_send(self, signal, *args):
        ...
        async_dispatcher_send(
            self.opp, f"{signal}_{self.config_entry.unique_id}", *args
        )
