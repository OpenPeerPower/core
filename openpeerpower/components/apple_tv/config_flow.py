...
class AppleTVConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input=None):
        ...
        if self.scan_result is None:
            self.scan_result, _ = await device_scan(None, self.opp.loop)
    ...
    async def async_find_device(self, allow_exist=False):
        ...
        self.scan_result, self.atv = await device_scan(
            self.target_device, self.opp.loop, cache=self.scan_result
        )
        if not self.atv:
    ...
    async def async_begin_pairing(self):
        ...
        self.pairing = await pair(
            self.atv, self.protocol, self.opp.loop, session=session
        )
        try:
...
class DeviceAlreadyConfigured(OpenPeerPowerError):
