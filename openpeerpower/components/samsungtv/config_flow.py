...
async def async_get_device_info(opp, bridge, host):
    """Fetch device info from bridge or websocket."""
    if bridge:
        return await opp.async_add_executor_job(bridge.device_info)

    return await opp.async_add_executor_job(_get_device_info, host)
...
class SamsungTVConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def _async_set_name_host_from_input(self, user_input):
        try:
            self._host = await self.opp.async_add_executor_job(
    ...
    async def async_step_user(self, user_input=None):
        ...
        if user_input is not None:
            await self._async_set_name_host_from_input(user_input)
            await self.opp.async_add_executor_job(self._try_connect)
            self._async_abort_entries_match({CONF_HOST: self._host})
    ...
    def _async_update_existing_host_entry(self, host):
        for entry in self._async_current_entries(include_ignore=False):
            ...
            if entry_kw_args:
                self.opp.config_entries.async_update_entry(entry, **entry_kw_args)
            return entry
        return None
    ...
    async def async_step_reauth(self, data):
        ...
        self._reauth_entry = self.opp.config_entries.async_get_entry(
    ...
    async def async_step_reauth_confirm(self, user_input=None):
        ...
        if user_input is not None:
            ...
            result = await self.opp.async_add_executor_job(bridge.try_connect)
            if result == RESULT_SUCCESS:
                ...
                await self.opp.config_entries.async_reload(self._reauth_entry.entry_id)
