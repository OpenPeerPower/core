...
class KeeneticFlowHandler(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input=None):
        ...
        if user_input is not None:
            ...
            try:
                router_info = await self.opp.async_add_executor_job(
...
class KeeneticOptionsFlowHandler(config_entries.OptionsFlow):
    ...
    async def async_step_init(self, _user_input=None):
        """Manage the options."""
        router: KeeneticRouter = self.opp.data[DOMAIN][self.config_entry.entry_id][
            ROUTER
        ]

        interfaces: List[InterfaceInfo] = await self.opp.async_add_executor_job(
