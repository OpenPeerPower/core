...
def _validate_input(opp: core.OpenPeerPower, data: dict, clear_token_file: bool):
...
async def validate_input(
    opp: core.OpenPeerPower, data: dict, clear_token_file: bool = False
):
    ...
    try:
        flume_devices = await opp.async_add_executor_job(
            _validate_input, opp, data, clear_token_file
        )
...
class ConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_reauth_confirm(self, user_input=None):
        ...
        if user_input is not None:
            ...
            try:
                await validate_input(self.opp, new_data, clear_token_file=True)
            except CannotConnect:
                errors["base"] = "cannot_connect"
            except InvalidAuth:
                errors[CONF_PASSWORD] = "invalid_auth"
