...
class HeosFlowHandler(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_ssdp(self, discovery_info):
        ...
        self.opp.data.setdefault(DATA_DISCOVERED_HOSTS, {})
        self.opp.data[DATA_DISCOVERED_HOSTS][friendly_name] = hostname
    ...
    async def async_step_user(self, user_input=None):
        ...
        if user_input is not None:
            ...
            host = self.opp.data[DATA_DISCOVERED_HOSTS].get(host, host)
            heos = Heos(host)
            try:
                await heos.connect()
                self.opp.data.pop(DATA_DISCOVERED_HOSTS)
        ...
        host_type = (
            str
            if not self.opp.data[DATA_DISCOVERED_HOSTS]
            else vol.In(list(self.opp.data[DATA_DISCOVERED_HOSTS]))
        )
