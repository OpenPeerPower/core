...
class RiscoOptionsFlowHandler(config_entries.OptionsFlow):
    ...
    async def async_step_risco_to_op.self, user_input=None):
        ...
        if user_input is not None:
            ...
            return await self.async_step_op.to_risco()

        risco_to_op.= self._data[CONF_RISCO_STATES_TO_HA]
        options = vol.Schema(
            {
    ...
    async def async_step_op.to_risco(self, user_input=None):
        ...
        options = {}
        risco_to_op.= self._data[CONF_RISCO_STATES_TO_HA]
        # we iterate over HA_STATES, instead of set(self._risco_to_op.values())
        # to ensure a consistent order
        for ha_state in HA_STATES:
            if ha_state not in risco_to_op.values():
                continue

            values = [
                risco_state
                for risco_state in RISCO_STATES
                if risco_to_op.risco_state] == op.state
            ]
