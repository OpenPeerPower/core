...
async def validate_input(
    opp: core.OpenPeerPower, data: dict[str, Any]
) -> dict[str, Any]:
    """Validate the user input allows us to connect."""
    session = async_get_clientsession(opp, verify_ssl=data[CONF_VERIFY_SSL])
...
class SmaConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(
        ...
        if user_input is not None:
            ...
            try:
                self._data[DEVICE_INFO] = await validate_input(self.opp, user_input)
    ...
    async def async_step_import(
        ...
        device_info = await validate_input(self.opp, import_config)
...
class CannotRetrieveDeviceInfo(exceptions.OpenPeerPowerError):
