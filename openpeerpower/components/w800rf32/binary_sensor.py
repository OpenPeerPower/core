...
from openpeerpower.const import CONF_DEVICE_CLASS, CONF_DEVICES, CONF_NAME
from openpeerpower.core import callback
from openpeerpower.helpers import config_validation as cv, event as evt
...
class W800rf32BinarySensor(BinarySensorEntity):
    ...
    def binary_sensor_update(self, event):
        ...
        if self.is_on and self._off_delay is not None and self._delay_listener is None:
            ...
            self._delay_listener = evt.async_track_point_in_time(
                self.opp, self._off_delay_listener, dt_util.utcnow() + self._off_delay
            )
    ...
    async def async_added_to_opp(self):
        ...
        async_dispatcher_connect(self.opp, self._signal, self.binary_sensor_update)
