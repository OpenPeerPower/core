...
from openpeerpower.components.mysensors import (
    CONF_DEVICE,
    DEFAULT_BAUD_RATE,
    DEFAULT_TCP_PORT,
    is_persistence_file,
...
class MySensorsConfigFlowHandler(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    def _check_topic_exists(self, topic: str) -> bool:
        for other_config in self.opp.config_entries.async_entries(DOMAIN):
    ...
    def _normalize_persistence_file(self, path: str) -> str:
        return os.path.realpath(os.path.normcase(self.opp.config.path(path)))
        ...
        if user_input is not None:
            ...
            if CONF_PERSISTENCE_FILE in user_input:
                try:
                    ...
                    for other_entry in self.opp.config_entries.async_entries(DOMAIN):
            ...
            if not errors and not await try_connect(self.opp, user_input):
