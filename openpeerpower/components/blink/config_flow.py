...
class BlinkConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
        ...
        if user_input is not None:
            ...
            try:
                await self.opp.async_add_executor_job(
                    validate_input, self.opp, self.auth
                )
    ...
    async def async_step_2fa(self, user_input=None):
        ...
        if user_input is not None:
            ...
            try:
                valid_token = await self.opp.async_add_executor_job(
...
class BlinkOptionsFlowHandler(config_entries.OptionsFlow):
    ...
    async def async_step_init(self, user_input=None):
        ...
        self.blink = self.opp.data[DOMAIN][self.config_entry.entry_id]
...
class Require2FA(exceptions.OpenPeerPowerError):
