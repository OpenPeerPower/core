...
class AsteriskData:
    ...
    def __init__(self, opp, host, port, password, config):
        ...
        dispatcher_connect(self.opp, SIGNAL_MESSAGE_REQUEST, self._request_messages)
        dispatcher_connect(self.opp, SIGNAL_CDR_REQUEST, self._request_cdr)
        dispatcher_connect(self.opp, SIGNAL_DISCOVER_PLATFORM, self._discover_platform)
    ...
    def _discover_platform(self, component):
        ...
        self.opp.async_create_task(
            discovery.async_load_platform(
                self.opp, "mailbox", component, {}, self.config
            )
        )
    ...
    def handle_data(self, command, msg):
        ...
        if command == CMD_MESSAGE_LIST:
            ...
            if not isinstance(old_messages, list):
                async_dispatcher_send(self.opp, SIGNAL_DISCOVER_PLATFORM, DOMAIN)
            async_dispatcher_send(self.opp, SIGNAL_MESSAGE_UPDATE, self.messages)
        elif command == CMD_MESSAGE_CDR:
            _LOGGER.debug(
                "AsteriskVM sent updated CDR list: Len %d", len(msg.get("entries", []))
            )
            self.cdr = msg["entries"]
            async_dispatcher_send(self.opp, SIGNAL_CDR_UPDATE, self.cdr)
        elif command == CMD_MESSAGE_CDR_AVAILABLE:
            if not isinstance(self.cdr, list):
                _LOGGER.debug("AsteriskVM adding CDR platform")
                self.cdr = []
                async_dispatcher_send(
                    self.opp, SIGNAL_DISCOVER_PLATFORM, "asterisk_cdr"
                )
            async_dispatcher_send(self.opp, SIGNAL_CDR_REQUEST)
        else:
            _LOGGER.debug(
