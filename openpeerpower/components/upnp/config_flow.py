...
class UpnpFlowHandler(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(
        ...
        # Discover devices.
        discoveries = [
            await Device.async_supplement_discovery(self.opp, discovery)
            for discovery in await Device.async_discover(self.opp)
        ]
    ...
    async def async_step_import(
        ...
        # Discover devices.
        self._discoveries = await Device.async_discover(self.opp)
        ...
        discovery = await Device.async_supplement_discovery(self.opp, discovery)
...
class UpnpOptionsFlowHandler(config_entries.OptionsFlow):
    ...
    async def async_step_init(self, user_input: Mapping = None) -> None:
        ...
        if user_input is not None:
            ...
            coordinator = self.opp.data[DOMAIN][DOMAIN_COORDINATORS][udn]
