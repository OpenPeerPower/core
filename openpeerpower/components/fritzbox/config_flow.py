...
from openpeerpower.config_entries import ConfigEntry, ConfigFlow
...
class FritzboxConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(
        ...
        if user_input is not None:
            ...
            result = await self.opp.async_add_executor_job(self._try_connect)
        ...
        for entry in self._async_current_entries():
            if entry.data[CONF_HOST] == host:
                if uuid and not entry.unique_id:
                    self.opp.config_entries.async_update_entry(entry, unique_id=uuid)
    ...
    async def async_step_reauth(self, data: dict[str, str]) -> FlowResult:
        ...
        entry = self.opp.config_entries.async_get_entry(self.context["entry_id"])
