...
async def async_setup(opp: OpenPeerPower, config: dict):
    ...
    def build_client() -> ProxmoxAPI:
        ...
        for entry in config[DOMAIN]:
            ...
            opp.data[PROXMOX_CLIENTS][host] = None
            ...
            opp.data[PROXMOX_CLIENTS][host] = proxmox_client

    await opp.async_add_executor_job(build_client)

    coordinators = opp.data[DOMAIN][COORDINATORS] = {}
    ...
    for host_config in config[DOMAIN]:
        ...
        proxmox_client = opp.data[PROXMOX_CLIENTS][host_name]
        ...
        for node_config in host_config["nodes"]:
            ...
            for vm_id in node_config["vms"]:
                coordinator = create_coordinator_container_vm(
                    opp, proxmox, host_name, node_name, vm_id, TYPE_VM
                )
            ...
            for container_id in node_config["containers"]:
                coordinator = create_coordinator_container_vm(
                    opp, proxmox, host_name, node_name, container_id, TYPE_CONTAINER
                )
...
def create_coordinator_container_vm(opp, proxmox, host_name, node_name, vm_id, vm_type):
    ...
    async def async_update_data():
        ...
        vm_status = await opp.async_add_executor_job(poll_api)
