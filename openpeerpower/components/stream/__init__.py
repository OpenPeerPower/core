...
async def async_setup.opp, config):
   ...
   .opp.data[DOMAIN][ATTR_STREAMS] = []
    ...
    # Setup HLS
   .opp.data[DOMAIN][ATTR_HLS_ENDPOINT] = async_setup_hls.opp)
...
class Stream:
    ...
    def endpoint_url(self) -> str:
        ...
        return self.opp.data[DOMAIN][ATTR_HLS_ENDPOINT].format(self.access_token)
    ...
    def hls_output(self) -> StreamOutput:
        ...
        if not self._hls:
            self._hls = HlsStreamOutput(self.opp)
            self._hls_timer = IdleTimer(self.opp, OUTPUT_IDLE_TIMEOUT, self._hls_idle)
    ...
    def _worker_finished(self):
        ...
        self.opp.loop.call_soon_threadsafe(self.stop)
    ...
    async def async_record(self, video_path, duration=30, lookback=5):
        ...
        self._recorder = RecorderOutput(self.opp)
        ...
        IdleTimer(self.opp, duration, save_recording).start()
