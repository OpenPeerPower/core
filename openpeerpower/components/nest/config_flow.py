...
class NestAuthError(OpenPeerPowerError):
...
class UnexpectedStateError(OpenPeerPowerError):
...
class NestFlowHandler(
    ...
    def is_sdm_api(self):
        ...
        return DOMAIN in self.opp.data and DATA_SDM in self.opp.data[DOMAIN]
    ...
    async def async_oauth_create_entry(self, data: dict) -> dict:
        ...
        existing_entries = self.opp.config_entries.async_entries(DOMAIN)
        if existing_entries:
            updated = False
            for entry in existing_entries:
                if updated:
                    await self.opp.config_entries.async_remove(entry.entry_id)
                    continue
                updated = True
                self.opp.config_entries.async_update_entry(
                    entry, data=data, unique_id=DOMAIN
                )
                await self.opp.config_entries.async_reload(entry.entry_id)
    ...
    async def async_step_user(self, user_input=None):
        ...
        if self.is_sdm_api():
            ...
            if self.opp.config_entries.async_entries(DOMAIN) and not self._reauth:
    ...
    async def async_step_link(self, user_input=None):
        ...
        flow = self.opp.data[DATA_FLOW_IMPL][self.flow_impl]
    ...
    async def async_step_import(self, info):
        ...
        if not await self.opp.async_add_executor_job(os.path.isfile, config_path):
            self.flow_impl = DOMAIN
            return await self.async_step_link()

        flow = self.opp.data[DATA_FLOW_IMPL][DOMAIN]
        tokens = await self.opp.async_add_executor_job(load_json, config_path)
