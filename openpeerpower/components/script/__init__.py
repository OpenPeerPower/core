...
from openpeerpower.components.blueprint import BlueprintInputs
...
from openpeerpower.helpers import extract_domain_configs
...
from openpeerpower.helpers.trace import trace_get, trace_path
...
@callback
def scripts_with_area(opp: OpenPeerPower, area_id: str) -> list[str]:
...
@callback
def areas_in_script(opp: OpenPeerPower, entity_id: str) -> list[str]:
...
async def _async_process_config(opp, config, component) -> bool:
    ...
    for config_key in extract_domain_configs(config, DOMAIN):
        ...
        for object_id, config_block in conf.items():
            ...
            entities.append(
                ScriptEntity(
                    opp, object_id, config_block, raw_config, raw_blueprint_inputs
                )
            )
    ...
    for entity in entities:
        ...
        async_set_service_schema(opp, DOMAIN, entity.object_id, service_desc)
...
class ScriptEntity(ToggleEntity):
    ...
    async def async_turn_on(self, **kwargs):
        ...
        self._changed.clear()
        self.opp.async_create_task(coro)
