...
async def async_get_service(opp, config, discovery_info=None):
    ...
    service = opp.data[DOMAIN][DATA_NOTIFY] = MobileAppNotificationService(opp)
    return service
...
class MobileAppNotificationService(BaseNotificationService):
    ...
    async def async_send_message(self, message="", **kwargs):
        ...
        local_push_channels = self.opp.data[DOMAIN][DATA_PUSH_CHANNEL]
        ...
        for target in targets:
            ...
            try:
                with async_timeout.timeout(10):
                    response = await async_get_clientsession(self._opp).post(
