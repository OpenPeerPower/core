...
def _set_update_interval(opp: OpenPeerPower, current_entry: ConfigEntry) -> timedelta:
...
async def async_setup_entry(opp: OpenPeerPower, entry: ConfigEntry) -> bool:
    ...
    if params:
        opp.config_entries.async_update_entry(entry, **params)

    api_class = ClimaCellV3 if entry.data[CONF_API_VERSION] == 3 else ClimaCellV4
    api = api_class(
        entry.data[CONF_API_KEY],
        entry.data.get(CONF_LATITUDE, opp.config.latitude),
        entry.data.get(CONF_LONGITUDE, opp.config.longitude),
    ...
    coordinator = ClimaCellDataUpdateCoordinator(
        ...
        entry,
        api,
        _set_update_interval(opp, entry),
    )
