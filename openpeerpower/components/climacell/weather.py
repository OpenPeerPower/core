...
class BaseClimaCellWeatherEntity(ClimaCellEntity, WeatherEntity):
    ...
    def _translate_condition(
        ...
        """Translate ClimaCell condition into an OPP condition."""
    ...
    def _forecast_dict(
        ...
        if use_datetime:
            translated_condition = self._translate_condition(
                condition, is_up(self.opp, forecast_dt)
            )
        else:
    ...
    def extra_state_attributes(self) -> Mapping[str, Any] | None:
        ...
        if wind_gust and self.opp.config.units.is_metric:
            wind_gust = round(
    ...
    def pressure(self):
        """Return the pressure."""
        if self.opp.config.units.is_metric and self._pressure:
            return round(
    ...
    def wind_speed(self):
        ...
        if self.opp.config.units.is_metric and self._wind_speed:
            return round(
    ...
    def visibility(self):
        ...
        if self.opp.config.units.is_metric and self._visibility:
            return round(
