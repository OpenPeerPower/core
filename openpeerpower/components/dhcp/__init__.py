...
from openpeerpower.loader import async_get_dhcp
from openpeerpower.util.network import is_invalid, is_link_local, is_loopback
...
class NetworkWatcher(WatcherBase):
    ...
    async def async_start(self):
        ...
        self._unsub = async_track_time_interval(
            self.opp, self.async_start_discover, SCAN_INTERVAL
        )
    ...
    def create_task(self, task):
        ...
        return self.opp.async_create_task(task)
...
class DeviceTrackerWatcher(WatcherBase):
    ...
    async def async_start(self):
        ...
        self._unsub = async_track_state_added_domain(
            self.opp, [DEVICE_TRACKER_DOMAIN], self._async_process_device_event
        )
        for state in self.opp.states.async_all(DEVICE_TRACKER_DOMAIN):
...
class DHCPWatcher(WatcherBase):
    ...
    async def async_start(self):
        ...
        try:
            await self.opp.async_add_executor_job(_verify_l2socket_setup, FILTER)
