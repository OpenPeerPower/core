...
from openpeerpower.const import CONF_WEBHOOK_ID, EVENT_OPENPEERPOWER_STOP
...
class ToonDataUpdateCoordinator(DataUpdateCoordinator[Status]):
    ...
    async def register_webhook(self, event: Optional[Event] = None) -> None:
        ...
        if CONF_WEBHOOK_ID not in self.entry.data:
            ...
            self.opp.config_entries.async_update_entry(self.entry, data=data)
        ...
        if self.opp.components.cloud.async_active_subscription():
            ...
            webhook_url = self.opp.components.webhook.async_generate_url(
                self.entry.data[CONF_WEBHOOK_ID]
            )
        ...
        self.opp.bus.async_listen_once(
            EVENT_OPENPEERPOWER_STOP, self.unregister_webhook
        )
