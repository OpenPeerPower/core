...
class IhcLight(IHCDevice, LightEntity):
    ...
    async def async_turn_on(self, **kwargs):
        ...
        if self._dimmable:
            await async_set_int(
                self.opp, self.ihc_controller, self.ihc_id, int(brightness * 100 / 255)
            )
        else:
            if self._ihc_on_id:
                await async_pulse(self.opp, self.ihc_controller, self._ihc_on_id)
            else:
                await async_set_bool(self.opp, self.ihc_controller, self.ihc_id, True)

    async def async_turn_off(self, **kwargs):
        """Turn the light off."""
        if self._dimmable:
            await async_set_int(self.opp, self.ihc_controller, self.ihc_id, 0)
        else:
            if self._ihc_off_id:
                await async_pulse(self.opp, self.ihc_controller, self._ihc_off_id)
            else:
                await async_set_bool(self.opp, self.ihc_controller, self.ihc_id, False)
