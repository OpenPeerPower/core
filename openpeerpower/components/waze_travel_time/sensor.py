...
from openpeerpower.core import Config, CoreState, OpenPeerPower
...
async def async_setup_platform(
    opp: OpenPeerPower, config: Config, async_add_entities, discovery_info=None
):
...
async def async_setup_entry(
    ...
    defaults = {
        ...
        CONF_UNITS: opp.config.units.name,
    ...
    async def async_added_to_opp(self) -> None:
        ...
        if self.opp.state != CoreState.running:
            self.opp.bus.async_listen_once(
                EVENT_OPENPEERPOWER_STARTED, self.first_update
            )
        else:
