...
async def get_api(opp: OpenPeerPower, host: str) -> Freepybox:
    """Get the Freebox API."""
    freebox_path = opp.helpers.storage.Store(STORAGE_VERSION, STORAGE_KEY).path

    if not os.path.exists(freebox_path):
        await opp.async_add_executor_job(os.makedirs, freebox_path)
...
class FreeboxRouter:
    ...
    async def setup(self) -> None:
        ...
        self._api = await get_api(self.opp, self._host)
        ...
        self._unsub_dispatcher = async_track_time_interval(
            self.opp, self.update_all, SCAN_INTERVAL
        )
