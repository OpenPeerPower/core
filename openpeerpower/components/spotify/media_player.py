    ...
    def wrapper(self, *args, **kwargs):
        try:
            ...
            if exc.reason == "NO_ACTIVE_DEVICE":
                raise OpenPeerPowerError("No active playback device found") from None
...
class SpotifyMediaPlayer(MediaPlayerEntity):
    ...
    def update(self) -> None:
        ...
        if not self._session.valid_token or self._spotify is None:
            run_coroutine_threadsafe(
                self._session.async_ensure_token_valid(), self.opp.loop
            ).result()
    ...
    async def async_browse_media(self, media_content_type=None, media_content_id=None):
        ...
        if media_content_type in [None, "library"]:
            return await self.opp.async_add_executor_job(library_payload)
