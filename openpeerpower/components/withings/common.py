...
class NotAuthenticatedError(OpenPeerPowerError):
    """Raise when not authenticated with the service."""


class ServiceError(OpenPeerPowerError):
...
class ConfigEntryWithingsApi(AbstractWithingsApi):
    """Withing API that uses OPP resources."""
...
def json_message_response(message: str, message_code: int) -> Response:
    ...
    return OpenPeerPowerView.json({"message": message, "code": message_code}, 200)
...
class BaseWithingsSensor(Entity):
    ...
    def should_poll(self) -> bool:
        """Return False to indicate OPP should not poll for changes."""
        return False
