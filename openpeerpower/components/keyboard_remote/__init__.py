...
class KeyboardRemote:
    ...
    def setup(self):
        ...
        self.opp.bus.async_listen_once(
            EVENT_OPENPEERPOWER_STOP, self.async_stop_monitoring
        )
    ...
    async def async_start_monitoring(self, event):
        ...
        await self.watcher.setup(self.opp.loop)
        ...
        self.monitor_task = self.opp.async_create_task(self.async_monitor_devices())
    ...
    class DeviceHandler:
        ...
        async def async_start_monitoring(self, dev):
            ...
            if self.monitor_task is None:
                self.dev = dev
                self.monitor_task = self.opp.async_create_task(
        ...
        async def async_stop_monitoring(self):
            ...
            if self.monitor_task is not None:
                try:
                    await self.opp.async_add_executor_job(self.dev.ungrab)
                except OSError:
                    pass
        ...
        async def async_monitor_input(self, dev):
            ...
            try:
                ...
                await self.opp.async_add_executor_job(dev.grab)
                async for event in dev.async_read_loop():
                    if event.type is ecodes.EV_KEY:
                        ...
                        if (
                            ...
                            repeat_tasks[event.code] = self.opp.async_create_task(
