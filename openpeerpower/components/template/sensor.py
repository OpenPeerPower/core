...
def extra_validation_checks(val):
    ...
    if CONF_TRIGGER in val:
        raise vol.Invalid(
            ...
            "See the template documentation for more information: https://www.openpeerpower.io/integrations/template/"
        )
...
async def async_setup_platform(opp, config, async_add_entities, discovery_info=None):
    ...
    if "coordinator" in discovery_info:
        async_add_entities(
            TriggerSensorEntity(opp, discovery_info["coordinator"], config)
...
class SensorTemplate(TemplateEntity, SensorEntity):
    ...
    def __init__(
        ...
        if friendly_name_template:
            friendly_name_template.opp = opp
            try:
