...
class MQTT:
    ...
    async def async_publish(
        ...
        async with self._paho_lock:
            msg_info = await self.opp.async_add_executor_job(
    ...
    async def _async_unsubscribe(self, topic: str) -> None:
        ...
        async with self._paho_lock:
            result: int = None
            result, mid = await self.opp.async_add_executor_job(
    ...
    async def _discovery_cooldown(self):
        ...
        self.opp.data[LAST_DISCOVERY] = now
        self._last_subscribe = now

        last_discovery = self.opp.data[LAST_DISCOVERY]
