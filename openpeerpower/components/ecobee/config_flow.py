...
class EcobeeFlowHandler(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input=None):
        ...
        errors = {}
        stored_api_key = (
            self.opp.data[DATA_ECOBEE_CONFIG].get(CONF_API_KEY)
            if DATA_ECOBEE_CONFIG in self.opp.data
            else ""
        )
        ...
        if user_input is not None:
            ...
            if await self.opp.async_add_executor_job(self._ecobee.request_pin):
    ...
    async def async_step_authorize(self, user_input=None):
        ...
        if user_input is not None:
            ...
            if await self.opp.async_add_executor_job(self._ecobee.request_tokens):
    ...
    async def async_step_import(self, import_data):
        ...
        """
        try:
            legacy_config = await self.opp.async_add_executor_job(
                load_json, self.opp.config.path(ECOBEE_CONFIG_FILENAME)
            )
            config = {
            ...
            return await self.async_step_user(
                user_input={
                    CONF_API_KEY: self.opp.data[DATA_ECOBEE_CONFIG].get(CONF_API_KEY)
                }
            )

        ecobee = Ecobee(config=config)
        if await self.opp.async_add_executor_job(ecobee.refresh_tokens):
