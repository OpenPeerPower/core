...
from openpeerpower.components.unifi.switch import BLOCK_SWITCH, POE_SWITCH
...
class UniFiController:
    ...
    def async_unifi_signalling_callback(self, signal, data):
        ...
        if signal == SIGNAL_CONNECTION_STATE:
            ...
            if (data == STATE_RUNNING and not self.available) or (
                ...
                async_dispatcher_send(self.opp, self.signal_reachable)

                if not self.available:
                    self.opp.loop.call_later(RETRY_TIMER, self.reconnect, True)
                else:
            ...
            if DATA_EVENT in data:
                ...
                async_dispatcher_send(
                    self.opp, self.signal_remove, data[DATA_CLIENT_REMOVED]
                )
                ...
                for key in data[DATA_DPI_GROUP]:
                    if self.api.dpi_groups[key].dpiapp_ids:
                        ...
                        async_dispatcher_send(self.opp, self.signal_remove, {key})

            elif DATA_DPI_GROUP_REMOVED in data:
                async_dispatcher_send(
                    self.opp, self.signal_remove, data[DATA_DPI_GROUP_REMOVED]
                )
    ...
    def update_wireless_clients(self):
        ...
        if new_wireless_clients:
            ...
            unifi_wireless_clients = self.opp.data[UNIFI_WIRELESS_CLIENTS]
    ...
    def _async_check_for_stale(self, *_) -> None:
        ...
        for unique_id, heartbeat_expire_time in self._heartbeat_time.items():
            if now > heartbeat_expire_time:
                async_dispatcher_send(
                    self.opp, f"{self.signal_heartbeat_missed}_{unique_id}"
                )
    ...
    async def async_config_entry_updated(opp, config_entry) -> None:
        ...
        """
        if not (controller := opp.data[UNIFI_DOMAIN].get(config_entry.entry_id)):
            return
    ...
    def reconnect(self, log=False) -> None:
        ...
        self.opp.loop.create_task(self.async_reconnect())
    ...
    async def async_reconnect(self) -> None:
        ...
        try:
            ...
            self.opp.loop.call_later(RETRY_TIMER, self.reconnect)
    ...
    async def async_reset(self):
        ...
        unload_ok = await self.opp.config_entries.async_unload_platforms(
