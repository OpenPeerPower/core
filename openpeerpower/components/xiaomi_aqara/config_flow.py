...
class XiaomiAqaraFlowHandler(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input=None):
        ...
        if self.host is not None and self.sid is not None:
            ...
            self.selected_gateway = await self.opp.async_add_executor_job(
                XiaomiGateway,
                self.host,
                self.sid,
                None,
        ...
        xiaomi = XiaomiGatewayDiscovery(self.opp.add_job, [], self.interface)
        try:
            await self.opp.async_add_executor_job(xiaomi.discover_gateways)
        except gaierror:
