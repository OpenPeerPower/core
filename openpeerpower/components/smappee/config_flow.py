...
from openpeerpower.const import CONF_HOST, CONF_IP_ADDRESS
...
class SmappeeFlowHandler(
    ...
    async def async_step_zeroconf_confirm(self, user_input=None):
        ...
        if helper.is_smappee_genius(serial_number):
            ...
            connect = await self.opp.async_add_executor_job(smappee_mqtt.start_attempt)
            if not connect:
    ...
    async def async_step_local(self, user_input=None):
        ...
        if logon is not None:
            advanced_config = await self.opp.async_add_executor_job(
            ...
            serial_number = await self.opp.async_add_executor_job(
                smappee_mqtt.start_and_wait_for_config
            )
            await self.opp.async_add_executor_job(smappee_mqtt.stop)
