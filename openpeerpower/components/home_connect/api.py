...
class ConfigEntryAuth(homeconnect.HomeConnectAPI):
    ...
    def get_devices(self):
        ...
        for app in appl:
            if app.type == "Dryer":
                device = Dryer(self.opp, app)
            elif app.type == "Washer":
                device = Washer(self.opp, app)
            elif app.type == "Dishwasher":
                device = Dishwasher(self.opp, app)
            elif app.type == "FridgeFreezer":
                device = FridgeFreezer(self.opp, app)
            elif app.type == "Oven":
                device = Oven(self.opp, app)
            elif app.type == "CoffeeMaker":
                device = CoffeeMaker(self.opp, app)
            elif app.type == "Hood":
                device = Hood(self.opp, app)
            elif app.type == "Hob":
                device = Hob(self.opp, app)
            else:
...
class HomeConnectDevice:
    ...
    def event_callback(self, appliance):
        ...
        dispatcher_send(self.opp, SIGNAL_UPDATE_ENTITIES, appliance.haId)
