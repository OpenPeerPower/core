...
class LeafDataStore:
    ...
    async def async_get_battery(self):
        ...
        try:
            ...
            request = await self.opp.async_add_executor_job(self.leaf.request_update)
            if not request:
            ...
            for attempt in range(MAX_RESPONSE_ATTEMPTS):
                ...
                check_result_info = await self.opp.async_add_executor_job(
                ...
                if check_result_info is not None:
                    ...
                    server_info = await self.opp.async_add_executor_job(
    ...
    async def async_set_climate(self, toggle):
        ...
        request = await self.opp.async_add_executor_job(set_function)
        for attempt in range(MAX_RESPONSE_ATTEMPTS):
            if attempt > 0:
                _LOGGER.debug(
            ...
            climate_result = await self.opp.async_add_executor_job(
