...
class OneWireFlowHandler(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_owserver(self, user_input=None):
        ...
        if user_input:
            ...
            if is_duplicate_owserver_entry(self.opp, user_input):
                return self.async_abort(reason="already_configured")

            self.onewire_config.update(user_input)

            try:
                info = await validate_input_owserver(self.opp, user_input)
    ...
    async def async_step_mount_dir(self, user_input=None):
        ...
        if user_input:
            ...
            try:
                info = await validate_input_mount_dir(self.opp, user_input)
            except InvalidPath:
