...
class ZWaveServices:
    ...
    def __init__(
        self, opp: OpenPeerPower, ent_reg: EntityRegistry, dev_reg: DeviceRegistry
    ) -> None:
        """Initialize with oppjobject."""
        self._opp = opp
    ...
    def async_register(self) -> None:
                ...
                if ATTR_ENTITY_ID in val:
                    nodes |= {
                        async_get_node_from_entity_id(
                            self._opp, entity_id, self._ent_reg, self._dev_reg
                        )
                ...
                if ATTR_DEVICE_ID in val:
                    nodes |= {
                        async_get_node_from_device_id(
                            self._opp, device_id, self._dev_reg
                        )
        ...
        def validate_multicast_nodes(val: dict[str, Any]) -> dict[str, Any]:
            ...
            if (
                not nodes
                and len(self._opp.config_entries.async_entries(const.DOMAIN)) > 1
            ):
                raise vol.Invalid(
    ...
    async def async_multicast_set_value(self, service: ServiceCall) -> None:
        ...
        if first_node:
            ...
            entry_id = self._opp.config_entries.async_entries(const.DOMAIN)[0].entry_id
            client = self._opp.data[const.DOMAIN][entry_id][const.DATA_CLIENT]
