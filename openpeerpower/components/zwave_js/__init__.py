...
async def async_setup_entry(  # noqa: C901
    ...
    async def async_on_node_ready(node: ZwaveNode) -> None:
        ...
        for disc_info in async_discover_values(node):
            ...
            async_dispatcher_send(
                opp, f"{DOMAIN}_{entry.entry_id}_add_{platform}", disc_info
            )
    ...
    async def async_on_node_added(node: ZwaveNode) -> None:
        ...
        node.once(
            "ready",
            lambda event: opp.async_create_task(async_on_node_ready(event["node"])),
        )
    ...
    async def start_platforms() -> None:
        ...
        # NOTE: This will not remove nodes that were removed when OPP was not running
        unsubscribe_callbacks.append(
            client.driver.controller.on(
...
async def disconnect_client(
    ...
    platform_setup_tasks = (
        opp.data[DOMAIN].get(entry.entry_id, {}).get(DATA_PLATFORM_SETUP, {}).values()
    )
...
async def async_ensure_addon_running(opp: OpenPeerPower, entry: ConfigEntry) -> None:
    ...
    if updates:
        opp.config_entries.async_update_entry(entry, data={**entry.data, **updates})
