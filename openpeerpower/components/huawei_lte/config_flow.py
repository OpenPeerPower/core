...
class ConfigFlowHandler(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(
        ...
        try:
            conn = await self.opp.async_add_executor_job(try_connect, user_input)
        ...
        if errors:
            await self.opp.async_add_executor_job(logout)
            return await self._async_show_user_form(
                user_input=user_input, errors=errors
            )
        ...
        title = self.context.get("title_placeholders", {}).get(
            CONF_NAME
        ) or await self.opp.async_add_executor_job(get_router_title, conn)
        await self.opp.async_add_executor_job(logout)
