...
from openpeerpower.const import HTTP_CREATED, HTTP_TOO_MANY_REQUESTS
...
class iOSNotificationService(BaseNotificationService):
    ...
    def targets(self):
        ...
        return ios.devices_with_push(self.opp)
    ...
    def send_message(self, message="", **kwargs):
        ...
        if not targets:
            targets = ios.enabled_push_ids(self.opp)

        if kwargs.get(ATTR_DATA) is not None:
            data[ATTR_DATA] = kwargs.get(ATTR_DATA)

        for target in targets:
            if target not in ios.enabled_push_ids(self.opp):
            ...
            if req.status_code != HTTP_CREATED:
                ...
                if req.status_code == HTTP_TOO_MANY_REQUESTS:
                    ...
                    log_rate_limits(self.opp, target, req.json(), 30)
                else:
                    _LOGGER.error(message)
            else:
                log_rate_limits(self.opp, target, req.json())
