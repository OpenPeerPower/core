...
from openpeerpower.const import HTTP_BAD_REQUEST, HTTP_INTERNAL_SERVER_ERROR
...
class iOSPushConfigView(OpenPeerPowerView):
...
class iOSConfigView(OpenPeerPowerView):
...
class iOSIdentifyDeviceView(OpenPeerPowerView):
