...
class HangoutsBot:
    ...
    async def async_connect(self):
        ...
        session = await self.opp.async_add_executor_job(
            get_auth,
        ...
        self.opp.loop.create_task(self._client.connect())

    def _on_connect(self):
        _LOGGER.debug("Connected!")
        self._connected = True
        dispatcher.async_dispatcher_send(self.opp, EVENT_HANGOUTS_CONNECTED)
    ...
    async def _on_disconnect(self):
        ...
        if self._connected:
            ...
            dispatcher.async_dispatcher_send(self.opp, EVENT_HANGOUTS_DISCONNECTED)
    ...
    async def _async_send_message(self, message, targets, data):
        ...
        if data:
            if data.get("image_url"):
                ...
                if self.opp.config.is_allowed_path(uri):
                    try:
