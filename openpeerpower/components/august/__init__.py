...
async def async_unload_entry(opp: OpenPeerPower, entry: ConfigEntry):
    ...
    opp.data[DOMAIN][entry.entry_id][DATA_AUGUST].async_stop()
...
async def async_setup_august(opp, config_entry, august_gateway):
    ...
    if CONF_PASSWORD in config_entry.data:
        ...
        opp.config_entries.async_update_entry(config_entry, data=config_data)

    await august_gateway.async_authenticate()

    opp.data.setdefault(DOMAIN, {})
    data = opp.data[DOMAIN][config_entry.entry_id] = {
        DATA_AUGUST: AugustData(opp, august_gateway)
    }
...
class AugustData(AugustSubscriberMixin):
    ...
    async def async_setup(self):
        ...
        self.activity_stream = ActivityStream(
            self._opp, self._api, self._august_gateway, self._house_ids, pubnub
        )
    ...
    async def _async_call_api_op_requires_bridge(
        ...
        try:
            ...
            raise OpenPeerPowerError(f"{device_name}: {err}") from err
