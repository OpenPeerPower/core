...
class FlowHandler(config_entries.ConfigFlow):
    ...
    async def async_step_auth(self, user_input=None):
        ...
        if user_input is not None:
            if await self.opp.async_add_executor_job(self._session.authorize):
        ...
        try:
            with async_timeout.timeout(10):
                auth_url = await self.opp.async_add_executor_job(self._get_auth_url)
            if not auth_url:
    ...
    async def async_step_import(self, user_input):
        ...
        if not await self.opp.async_add_executor_job(
            os.path.isfile, self.opp.config.path(TELLDUS_CONFIG_FILE)
        ):
            return await self.async_step_user()

        conf = await self.opp.async_add_executor_job(
            load_json, self.opp.config.path(TELLDUS_CONFIG_FILE)
        )
        host = next(iter(conf))
