...
class RflinkDevice(Entity):
    ...
    async def async_added_to_opp(self):
        ...
        if (
            tmp_entity
            in self.opp.data[DATA_ENTITY_LOOKUP][EVENT_KEY_COMMAND][self._device_id]
        ):
            self.opp.data[DATA_ENTITY_LOOKUP][EVENT_KEY_COMMAND][
                self._device_id
            ].remove(tmp_entity)

        # Register id and aliases
        self.opp.data[DATA_ENTITY_LOOKUP][EVENT_KEY_COMMAND][self._device_id].append(
            self.entity_id
        )
        if self._group:
            self.opp.data[DATA_ENTITY_GROUP_LOOKUP][EVENT_KEY_COMMAND][
                self._device_id
            ].append(self.entity_id)
        # aliases respond to both normal and group commands (allon/alloff)
        if self._aliases:
            for _id in self._aliases:
                self.opp.data[DATA_ENTITY_LOOKUP][EVENT_KEY_COMMAND][_id].append(
                    self.entity_id
                )
                self.opp.data[DATA_ENTITY_GROUP_LOOKUP][EVENT_KEY_COMMAND][_id].append(
                    self.entity_id
                )
        ...
        self.async_on_remove(
            async_dispatcher_connect(
                self.opp, SIGNAL_AVAILABILITY, self._availability_callback
            )
        )
        self.async_on_remove(
            async_dispatcher_connect(
...
class RflinkCommand(RflinkDevice):
    ...
    async def _async_send_command(self, cmd, repetitions):
        ...
        if not self.is_connected():
            raise OpenPeerPowerError("Cannot send command, not connected!")
        ...
        if repetitions > 1:
            self._repetition_task = self.opp.async_create_task(
