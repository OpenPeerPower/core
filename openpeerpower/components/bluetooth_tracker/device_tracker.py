...
from openpeerpower.const import CONF_DEVICE_ID
...
async def get_tracking_devices(opp: OpenPeerPower) -> tuple[set[str], set[str]]:
    """
    ...
    devices = await async_load_config(yaml_path, opp, timedelta(0))
    ...
    async def perform_bluetooth_update() -> None:
        ...
        try:
            ...
            for mac in devices_to_track:
                friendly_name = await opp.async_add_executor_job(lookup_name, mac)
                ...
                tasks.append(see_device(opp, async_see, mac, friendly_name, rssi))
